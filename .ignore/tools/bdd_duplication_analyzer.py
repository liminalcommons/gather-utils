...
class BDDDuplicationAnalyzer:
    ...
    def _load_features(self):
        ...
        logger.info(
            f"Loaded {len(self.features)} features with {sum(len(f.scenarios) for f in self.features)} scenarios"
        )
    ...
    def _parse_feature_file(self, file_path: Path) -> Feature:
        ...
        for i, line in enumerate(lines):
            ...
            if (
                ...
                and not line.strip().startswith("Background:")
                and not line.strip().startswith("Scenario:")
                and not line.strip().startswith("Scenario Outline:")
            ):
            ...
            # Parse scenario
            if line.strip().startswith("Scenario:") or line.strip().startswith(
                "Scenario Outline:"
            ):
                in_background = False
                if current_scenario:
        ...
        for feature in self.features:
            for scenario in feature.scenarios:
                ...
                if normalized_content not in scenario_dict:
                    ...
                    scenario_dict[normalized_content].append(
                        (feature, scenario)
                    )
        ...
        for content, instances in scenario_dict.items():
            if len(instances) > 1:
                ...
                for target_feature, target_scenario in instances[1:]:
                    ...
                    logger.debug(
                        f"Found exact duplicate: {source_feature.path.name}:{source_scenario.name} = {target_feature.path.name}:{target_scenario.name}"
                    )

        logger.info(
            f"Found {sum(1 for d in self.duplicates if d.type == 'exact')} exact duplicates"
        )
            ...
            for scenario1 in feature1.scenarios:
                ...
                for feature2 in self.features[i:]:
                    ...
                    for scenario2 in feature2.scenarios:
                        ...
                        content1 = re.sub(
                            r"\s+", " ", scenario1.content.strip()
                        )
                        content2 = re.sub(
                            r"\s+", " ", scenario2.content.strip()
                        )
                        ...
                        if similarity >= SIMILARITY_THRESHOLD:
                            ...
                            self.duplicates.append(duplicate)
                            logger.debug(
                                f"Found semantic duplicate: {feature1.path.name}:{scenario1.name} ~ {feature2.path.name}:{scenario2.name} ({similarity:.1%})"
                            )

        logger.info(
            f"Found {sum(1 for d in self.duplicates if d.type == 'semantic')} semantic duplicates"
        )
        ...
        for i, feature1 in enumerate(self.features):
            ...
            for feature2 in self.features[i + 1 :]:
                ...
                name_similarity = difflib.SequenceMatcher(
                    None, feature1.name, feature2.name
                ).ratio()
                ...
                if (
                    ...
                    or tag_overlap >= TAG_OVERLAP_THRESHOLD
                ):
                    ...
                    duplicate = Duplicate(
                        ...
                        source=feature1,
                        target=feature2,
                        similarity=max(name_similarity, tag_overlap)
                        * 100,  # Use the higher similarity as the overall measure
                    ...
                    # Log the details
                    common_tag_str = (
                        ", ".join(tags1.intersection(tags2))
                        if tags1 and tags2
                        else "none"
                    )
                    logger.debug(
                        f"Found overlapping features: {feature1.path.name} ~ {feature2.path.name} (overlap type: {overlap_type}, common tags: {common_tag_str})"
                    )

        logger.info(
            f"Found {sum(1 for d in self.duplicates if d.type == 'overlap')} overlapping features"
        )
    ...
    def _find_tag_inconsistencies(self):
        ...
        for feature in self.features:
            for scenario in feature.scenarios:
                ...
                normalized_name = re.sub(
                    r"\b(the|a|an|in|on|for|to|with|and|or|of|that|this|as)\b",
                    "",
                    scenario.name.lower(),
                )
                ...
                scenario_name_groups[normalized_name].append(
                    (feature, scenario)
                )
        ...
        for name, scenarios in scenario_name_groups.items():
            ...
            for feature, scenario in scenarios:
                ...
                req_tags_in_scenario = [
                    tag for tag in scenario.tags if req_pattern.match(tag)
                ]
                ...
                rel_tags_in_scenario = [
                    tag for tag in scenario.tags if rel_pattern.match(tag)
                ]
            ...
            for feature, scenario in scenarios:
                ...
                if (
                    req_tags_in_scenario
                    and req_tags_in_scenario[0] not in req_tags
                ):
                    inconsistent_tags = True
            ...
            if inconsistent_tags and len(scenarios) > 1:
                ...
                for feature, scenario in scenarios[1:]:
                    ...
                    if extra_tags:
                        inconsistency_desc.append(
                            f"Extra tags: {', '.join(extra_tags)}"
                        )

                    recommendation = (
                        "Consider standardizing tags across similar scenarios."
                    )
                    if req_tags_present:
                        recommendation += (
                            f" Requirement tags found: {', '.join(req_tags)}."
                        )
                    if rel_tags_present:
                        recommendation += (
                            f" Release tags found: {', '.join(rel_tags)}."
                        )
                    ...
                    self.duplicates.append(duplicate)
                    logger.debug(
                        f"Found tag inconsistency: {base_feature.path.name}:{base_scenario.name} vs {feature.path.name}:{scenario.name}"
                    )

        logger.info(
            f"Found {sum(1 for d in self.duplicates if d.type == 'tag_inconsistency')} tag inconsistencies"
        )
    ...
    def generate_report(self, output_file: Path) -> str:
        ...
        for issue_type in [
            ...
            for i, issue in enumerate(issues, 1):
                report += (
                    f"### {i}. {issue.source.name} vs {issue.target.name}\n\n"
                )
                ...
                if issue_type in ["exact", "semantic"]:
                    for j, (src, tgt) in enumerate(
                        zip(issue.source_scenarios, issue.target_scenarios), 1
                    ):
    ...
    def generate_merge_plan(self, output_file: Path = MERGE_PLAN_FILE):
        ...
        now = datetime.datetime.now()
        merge_plan = {"generated_date": now.isoformat(), "merge_groups": []}
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze BDD feature files for duplication and inconsistencies."
    )
    parser.add_argument(
        "--output-file",
        "-o",
...
def main():
    ...
    logger.info(
        f"Found {len(analyzer.duplicates)} issues. See report at {output_file}"
    )
    return 0
