"""
Tool: Bdd Feature Reorganizer
Created: 2025-03-21
Author: Development Team
Status: Active
Purpose: Supports BDD testing and development workflows
Dependencies: logging, shutil, dataclasses
Lifecycle:
    ...
    def suggested_filename(self) -> str:
        ...
        if not self.domain or not self.functionality:
            ...
            functionality = self.functionality or (
                parts[1] if len(parts) > 1 else "general"
            )
...
class ReorganizationPlan:
    ...
    def execute(self, dry_run: bool = False) -> bool:
        ...
        logger.info(
            f"Executing plan: {self.action} {self.source_path} to {self.target_path}"
        )
...
class FeatureReorganizer:
    ...
    def _parse_feature_file(self, file_path: Path) -> Optional[Feature]:
        ...
        try:
            ...
            domain, functionality = self._extract_domain_functionality(
                file_path, feature_tags
            )
            ...
            logger.info(
                f"Parsed feature: {feature_name} ({domain}/{functionality})"
            )
            return feature
    ...
    def _extract_domain_functionality(
    ...
    def create_reorganization_plans(self):
        ...
        for feature in self.features:
            ...
            if (
                str(current_directory) == target_directory
                and source_path.name == target_filename
            ):
                logger.info(
                    f"Feature {source_path.name} already in correct location"
                )
                continue
            ...
            if plan.execute(dry_run=self.dry_run):
                ...
                if self.delete_source and not self.dry_run:
                    try:
                        ...
                        logger.error(
                            f"Failed to delete source file {plan.source_path}: {e}"
                        )

        logger.info(
            f"Successfully executed {success_count}/{len(self.plans)} plans"
        )
    ...
    def reorganize_step_definitions(self):
        ...
        if not self.steps_dir:
            logger.info(
                "No steps directory provided, skipping step definition reorganization"
            )
            return
        ...
        for feature_path, step_file in feature_to_steps.items():
            ...
            target_directory = feature.suggested_directory
            target_filename = feature.suggested_filename.replace(
                ".feature", "_steps.py"
            )
            target_path = (
                self.output_dir / "steps" / target_directory / target_filename
            )
        ...
        logger.info(
            f"Created {len(step_plans)} step definition reorganization plans"
        )
            ...
            if plan.execute(dry_run=self.dry_run):
                ...
                if self.delete_source and not self.dry_run:
                    try:
                        os.remove(plan.source_path)
                        logger.info(
                            f"Deleted source step file {plan.source_path}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to delete source step file {plan.source_path}: {e}"
                        )

        logger.info(
            f"Successfully executed {success_count}/{len(step_plans)} step plans"
        )
    ...
    def generate_report(self, output_file: str):
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- Features requiring reorganization: {len(self.plans)}\n"
            )
            ...
            for i, plan in enumerate(self.plans):
                f.write(
                    f"### Plan {i+1}: {plan.action.capitalize()} {plan.feature.name}\n\n"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Reorganize BDD feature files according to conventions"
    )
    parser.add_argument(
        "--features-dir",
        required=True,
    ...
    parser.add_argument(
        "--output-dir", required=True, help="Directory for reorganized output"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--delete-source",
        action="store_true",
        help="Delete source files after reorganization",
    )
