"""
Tool: Bdd Tag Standardizer
Created: 2025-03-21
Author: Development Team
Status: Active
Purpose: Standardize tags in BDD feature files
...
class Scenario:
    ...
    def requirement_tags(self) -> List[str]:
        ...
        return [
            ...
            if re.match(TAG_PATTERNS["requirement"], tag)
        ]
    ...
    def release_tags(self) -> List[str]:
        ...
        return [
            tag for tag in self.tags if re.match(TAG_PATTERNS["release"], tag)
        ]
    ...
    def component_tags(self) -> List[str]:
        ...
        return [
            ...
            for tag in self.tags
            if not any(
                re.match(pattern, tag)
                for pattern in [
                    TAG_PATTERNS["requirement"],
                    TAG_PATTERNS["release"],
                    TAG_PATTERNS["process"],
                ]
            )
        ]
    ...
    def process_tags(self) -> List[str]:
        ...
        return [
            tag for tag in self.tags if re.match(TAG_PATTERNS["process"], tag)
        ]
...
class TagStandardizer:
    ...
    def __init__(
        ...
        self.tag_mapping_file = (
            Path(tag_mapping_file) if tag_mapping_file else None
        )
        self.component_map_file = (
            Path(component_map_file) if component_map_file else None
        )
    ...
    def _load_component_map(self):
        ...
        logger.info(
            f"Loading component mapping from {self.component_map_file}"
        )
    ...
    def _parse_feature_file(self, file_path: Path) -> Optional[Feature]:
        ...
        try:
            ...
            feature_match = re.search(
                r"Feature:\s*(.+?)$", content, re.MULTILINE
            )
            if not feature_match:
                logger.warning(
                    f"Could not find Feature declaration in {file_path}"
                )
                return None
            ...
            logger.info(
                f"Parsed feature {feature_name} with {len(feature.scenarios)} scenarios"
            )
            return feature
    ...
    def analyze_tags(self):
        ...
        logger.info(
            f"Analysis complete: Found {len(self.tag_modifications)} tag modifications needed"
        )
    ...
    def _analyze_scenario_tags(
        self, scenario: Scenario, feature: Feature
    ) -> List[TagModification]:
        ...
        for tag in scenario.requirement_tags:
            if not re.match(TAG_PATTERNS["requirement"], tag):
                ...
                if replacement:
                    new_tags = [
                        t if t != tag else replacement for t in scenario.tags
                    ]
    ...
    def _find_tag_replacement(
        self, original_tag: str, tag_type: str
    ) -> Optional[str]:
    ...
    def apply_modifications(self):
        ...
        logger.info(
            f"Applying {len(self.tag_modifications)} tag modifications"
        )
        ...
        for feature_path, mods in modifications_by_feature.items():
            # Find the feature
            feature = next(
                (f for f in self.features if str(f.path) == feature_path), None
            )
            if not feature:
                logger.warning(
                    f"Could not find feature for path {feature_path}"
                )
                continue
    ...
    def _apply_to_feature(
        self, feature: Feature, modifications: List[TagModification]
    ):
        """Apply modifications to a specific feature."""
        logger.info(
            f"Applying {len(modifications)} modifications to {feature.path}"
        )
        ...
        for scenario in feature.scenarios:
            ...
            scenario_outline_position = content.find(
            ...
            position = (
                ...
                else scenario_outline_position
            )

            if position == -1:
                logger.warning(
                    f"Could not find scenario {scenario.name} in content"
                )
                continue
            ...
            if tag_start == -1:
                ...
                final_tags = self._compute_final_tags(
                    scenario.tags, scenario_mods
                )
                ...
                # Insert the tags
                content = (
                    content[:line_start] + tag_section + content[line_start:]
                )
            else:
                ...
                content = (
                    content[:tag_start] + new_tag_section + content[tag_end:]
                )

        # Save the modified content
        output_path = (
            self.output_dir / feature.path.name
            if self.output_dir
            else feature.path
        )
    ...
    def _compute_final_tags(
        self, original_tags: List[str], modifications: List[TagModification]
    ) -> List[str]:
        ...
        for mod in modifications:
            if mod.action == "add":
                ...
                current_tags = [
                    tag for tag in current_tags if tag not in mod.old_tags
                ]
                ...
                tag_mapping = {
                    old: new
                    for old, new in zip(mod.old_tags, mod.new_tags)
                    if old != new
                }

                current_tags = [
                    tag_mapping.get(tag, tag) for tag in current_tags
                ]
    ...
    def generate_report(self, output_file: str):
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- Total scenarios analyzed: {sum(len(feature.scenarios) for feature in self.features)}\n"
            )
            ...
            for mod in self.tag_modifications:
                action_counts[mod.action] = (
                    action_counts.get(mod.action, 0) + 1
                )

                # Simplified reason category
                category = (
                    "requirement"
                    if "requirement" in mod.reason
                    else (
                        "release"
                        if "release" in mod.reason
                        else (
                            "component"
                            if "component" in mod.reason
                            else (
                                "domain" if "domain" in mod.reason else "other"
                            )
                        )
                    )
                )
            ...
            for feature_path, mods in mods_by_feature.items():
                feature = next(
                    (f for f in self.features if str(f.path) == feature_path),
                    None,
                )
                if not feature:
                    continue
                ...
                for i, mod in enumerate(mods):
                    f.write(
                        f"#### Modification {i+1}: {mod.action.capitalize()}\n\n"
                    )
            ...
            f.write(
                "When adding tags to BDD features, follow these guidelines:\n\n"
            )

            f.write(
                "1. **Requirement Tags**: Always include a requirement tag in the format `@REQ-[domain]-[id]`\n"
            )
            f.write("   - Example: `@REQ-API-123`\n\n")

            f.write(
                "2. **Release Tags**: Include a release tag in the format `@REL-[version]`\n"
            )
            f.write("   - Example: `@REL-1`\n\n")

            f.write(
                "3. **Component Tags**: Include at least one component tag\n"
            )
            f.write("   - Example: `@API`, `@CLI`, `@MODEL`, etc.\n\n")

            f.write(
                "4. **Process Tags**: Include process tags when relevant in the format `@PROC-[process]`\n"
            )
            f.write("   - Example: `@PROC-DEPLOY`, `@PROC-SECURITY`\n\n")

            f.write(
                "5. **Consistency**: Use consistent capitalization and formatting\n\n"
            )

            f.write(
                "6. **One Tag Per Line**: Place each tag on its own line for readability\n\n"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Standardize tags across BDD feature files"
    )
    parser.add_argument(
        "--features-dir",
        required=True,
    ...
    parser.add_argument(
        "--output-dir",
        help="Directory for output (if not specified, features will be modified in place)",
    )
    parser.add_argument("--tag-mapping", help="JSON file with tag mappings")
    parser.add_argument(
        "--component-map", help="JSON file with component mappings"
    )
    parser.add_argument(
        "--report", help="Path to write standardization report"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
