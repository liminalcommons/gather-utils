    ...
    try:
        ...
        if portals:
            ...
            for i, portal in enumerate(portals, 1):
                ...
                print(
                    f"  Position: ({portal.get('x', '?')}, {portal.get('y', '?')})"
                )
                print(
                    f"  Target: {portal.get('targetMap', '?')} at ({portal.get('targetX', '?')}, {portal.get('targetY', '?')})"
                )
                ...
                if "width" in portal or "height" in portal:
                    print(
                        f"  Size: {portal.get('width', 1)}x{portal.get('height', 1)}"
                    )

        # Save portals to a separate file
        output_path = Path(file_path).with_name(
            f"portals_{Path(file_path).stem}.json"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze a Gather.town map file and extract portal information"
    )
