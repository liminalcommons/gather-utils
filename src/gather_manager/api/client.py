...
from gather_manager.models.space import Map, MapData, Object, Portal, Space
...
class GatherClient:
    ...
    # Role constants
    ROLE_MEMBER = (
        "MEMBER"  # Current role name (previously was "GENERAL_MEMBER")
    )
    ...
    def __init__(
        self, api_key: Optional[str] = None, base_url: str = DEFAULT_BASE_URL
    ):
        ...
        if not self.api_key:
            raise ValueError(
                "API key is required. Provide it directly or set GATHER_API_KEY environment variable."
            )
        ...
        try:
            ...
            if response.status_code == 404:
                ...
                if "space" in endpoint:
                    ...
                    if (
                        "maps" in endpoint
                        and len(endpoint.split("/maps/")) > 1
                    ):
                        map_id = endpoint.split("/maps/")[1].split("/")[0]
                        error_context = (
                            f"Map '{map_id}' not found in space '{space_id}'"
                        )
                raise GatherApiError(
                    f"404 Not Found: {error_context}",
                    status_code=404,
                    endpoint=endpoint,
                )
            ...
            raise GatherApiError(
                error_msg, status_code=status_code, endpoint=endpoint
            ) from e

        except requests.exceptions.ConnectionError as e:
            error_msg = (
                f"Connection error while accessing Gather API: {str(e)}"
            )
            logger.error(error_msg)
            ...
            error_msg = (
                f"Timeout while waiting for Gather API response: {str(e)}"
            )
            logger.error(error_msg)
            raise GatherApiError(error_msg, endpoint=endpoint) from e
    ...
    def create_space(
        self, name: str, source_space: str, reason: Optional[str] = None
    ) -> Space:
        ...
        data = {"name": name, "sourceSpace": formatted_source_space}

        if reason:
            data["reason"] = reason

        response = self._request(
            "POST", f"api/{self.API_VERSION}/spaces", data=data
        )
    ...
    def get_space(self, space_id: str) -> Space:
        ...
        data = self._request(
            "GET", f"api/{self.API_VERSION}/spaces/{formatted_space_id}"
        )
    ...
    def get_spaces(self, role: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        return self._request(
            "GET", f"api/{self.API_VERSION}/users/me/spaces", params=params
        )
    ...
    def update_map(
        ...
        space_id: str,
        map_id: str,
        map_data: Union[MapData, Dict[str, Any]],
    ) -> MapData:
        ...
        if isinstance(map_data, MapData):
            ...
            raise TypeError(
                "map_data must be a MapData instance or a dictionary"
            )
    ...
    def update_map_background(
        self, space_id: str, map_id: str, background: str
    ) -> MapData:
    ...
    def update_map_objects(
        self, space_id: str, map_id: str, objects: List[Object]
    ) -> MapData:
    ...
    def get_portals(self, space_id: str, map_id: str) -> List[Object]:
        ...
        for obj in objects:
            ...
            if obj.properties and any(
                k in str(obj.properties).lower()
                for k in ["portal", "target", "teleport", "warp"]
            ):
                portals.append(obj)
                continue

            # Case 4: Object has a specific integer type that we've identified as portals
            if isinstance(obj.type, int) and obj.type in [
                4,
                5,
                6,
                7,
            ]:  # Potential portal type values
                portals.append(obj)
                continue
    ...
    def get_user_id_by_email(self, email: str) -> str:
        ...
        try:
            data = self._request(
                "GET",
                f"api/{self.API_VERSION}/user-id",
                params={"email": email},
            )
    ...
    def add_user_to_space(
        self, space_id: str, email: str, role: str = ROLE_MEMBER
    ) -> Dict[str, Any]:
    ...
    def get_space_users(
        self, space_id: str, role: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def create_spawn_token(
        ...
        data = {"room": map_id, "spawn": spawn_name, "type": "SpawnTile"}
