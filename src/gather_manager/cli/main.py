...
def explore(
    ...
    try:
        ...
        if not explorer.check_space_access(space_id):
            console.print(f"[bold red]Error:[/] Cannot access space {space_id}. Please check your API key and permissions.")
...
def list_spaces(
    role: Optional[str] = typer.Option(
        None,
        help="Filter spaces by role (ADMIN, BUILDER, MEMBER, MODERATOR)"
    )
):
    """
    ...
    try:
        ...
        if role:
            ...
            if role_upper not in role_constants:
                console.print(f"[bold yellow]Warning:[/] Unknown role '{role}'. Using as-is.")
                role_to_use = role
            else:
        ...
        if not spaces:
            console.print(f"[yellow]No spaces found{' with role ' + role if role else ''}[/]")
            return
        
        table = Table(title=f"Accessible Spaces{' (Role: ' + role + ')' if role else ''}")
        table.add_column("#", style="dim")
...
def manage_users(
    ...
    list_users: bool = typer.Option(
        False,
        "--list", "-l",
        help="List users in the space"
    ),
    add_user: bool = typer.Option(
        False,
        "--add", "-a",
        help="Add a user to the space"
    ),
    remove_user: bool = typer.Option(
        False,
        "--remove", "-r",
        help="Remove a user from the space"
    ),
    email: Optional[str] = typer.Option(
        None,
        help="Email of the user to add or remove"
    ),
    role: Optional[str] = typer.Option(
        "MEMBER",
    ...
    try:
        ...
        if add_user:
            ...
            console.print(f"[bold]Adding user[/] {email} to space {space_id} with role {role_upper}")
            client.add_user_to_space(space_id, email, role=role_to_use)
            console.print(f"[green]Successfully added user {email} with role {role_upper}[/]")
            return
        ...
        if remove_user:
            ...
            if Confirm.ask(f"[bold yellow]Are you sure you want to remove user {email} from space {space_id}?[/]"):
                console.print(f"[bold]Removing user[/] {email} from space {space_id}")
                client.remove_user_from_space(space_id, email)
                console.print(f"[green]Successfully removed user {email}[/]")
            return
        
        # If no operation specified, show help
        console.print("[yellow]Please specify an operation: --list, --add, or --remove[/]")
...
def create_spawn_token(
    ...
    spawn_name: str = typer.Option(
        ...,
        help="Name of the spawn point"
    ),
    ...
    try:
        ...
        console.print(f"[bold]Creating spawn token for[/] {spawn_name} in map {map_id}")
        result = client.create_spawn_token(space_id, map_id, spawn_name, expires_in_seconds=expires_in)
        
        console.print(f"[green]Successfully created spawn token:[/]")
        console.print(f"  [cyan]Token:[/] {result.get('spawnToken', 'Unknown')}")
        if "url" in result:
...
def validate_portals(
    ...
    api_key: str = typer.Option(
        ..., 
        envvar="GATHER_API_KEY",
        help="Gather.town API key"
    )
):
    ...
    if portal_details:
        for portal in portal_details:
            ...
            if portal.get("is_valid", False):
                console.print(f"Destination: {portal['target_map']} ({portal['target_x']}, {portal['target_y']})")
                console.print("Valid: [green]Yes[/green]")
            else:
                if "target_map" in portal and portal["target_map"] is not None:
                    console.print(f"Destination: {portal['target_map']} ({portal.get('target_x', '?')}, {portal.get('target_y', '?')})")
                else:
                    console.print("Destination: [italic]Not specified[/italic]")
...
def export_portals(
    ...
    with console.status(f"Exporting portal data to {format.upper()} format..."):
        # Create API client
        api_client = GatherClient(api_key=api_key)
        ...
        # Export portals
        file_path = portal_service.export_portals(format=format, output_dir=output_dir)
