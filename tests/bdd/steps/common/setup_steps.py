...
def step_given_have_existing_development_system(context):
    ...
    assert (
        context.has_development_system
    ), "Should have an existing development system"
...
def step_given_have_agent_and_project_systems(context):
    ...
    assert (
        context.has_agent_and_project_systems
    ), "Should have agent and project systems"
...
def step_given_have_established_release_standards(context):
    ...
    assert (
        context.has_release_standards
    ), "Should have established release standards"
...
def step_given_release_has_been_completed(context, number):
    ...
    assert (
        number in context.completed_releases
    ), f"Release {number} should be completed"
...
def step_given_bdd_release_has_been_completed(context, number):
    ...
    # For testing, we just set up a flag
    context.completed_bdd_releases = getattr(
        context, "completed_bdd_releases", set()
    )
    context.completed_bdd_releases.add(number)
    assert (
        number in context.completed_bdd_releases
    ), f"BDD Release {number} should be completed"
