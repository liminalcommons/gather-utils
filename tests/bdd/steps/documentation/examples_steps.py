...
"""
    assert (
        "Portal Explorer" in context.readme_content
    ), "README should include the project name"
...
"""
    assert (
        "Portal Explorer CLI" in context.cli_docs
    ), "CLI docs should include the CLI name"
...
"""
    assert (
        "Portal Explorer API" in context.api_docs
    ), "API docs should include the API name"
...
""",
    ...
    assert (
        len(context.example_scripts) >= 2
    ), "Should have at least 2 example scripts"
...
def step_given_am_viewing_notebook_examples(context):
    ...
    assert (
        len(context.notebook_examples) >= 2
    ), "Should have at least 2 notebook examples"
...
"""
    assert (
        "Error Handling" in context.error_docs
    ), "Error docs should include the title"
...
def step_then_see_project_overview(context):
    ...
    assert (
        "A tool for exploring" in context.readme_content
    ), "README should include project description"
...
def step_then_see_installation_instructions(context):
    ...
    assert (
        "Installation" in context.readme_content
    ), "README should include Installation section"
    assert (
        "pip install" in context.readme_content
...
def step_then_see_basic_usage_examples(context):
    ...
    assert (
        "Usage" in context.readme_content
    ), "README should include Usage section"
    assert (
        "list-maps" in context.readme_content
    ), "README should include list-maps example"
    assert (
        "explore" in context.readme_content
    ), "README should include explore example"
...
def step_then_see_api_key_configuration(context):
    ...
    assert (
        "API Key Configuration" in context.readme_content
    ), "README should include API key section"
    assert (
        "GATHER_API_KEY" in context.readme_content
    ), "README should mention environment variable"
...
def step_then_see_available_commands_list(context):
    ...
    assert (
        "Commands" in context.cli_docs
    ), "CLI docs should include Commands section"
    assert (
        "List Maps" in context.cli_docs
    ), "CLI docs should include List Maps command"
    assert (
        "Explore" in context.cli_docs
    ), "CLI docs should include Explore command"
    assert (
        "Validate" in context.cli_docs
    ), "CLI docs should include Validate command"
...
def step_then_see_command_examples(context):
    ...
    assert (
        "gather-explorer list-maps" in context.cli_docs
    ), "CLI docs should include list-maps example"
    assert (
        "gather-explorer explore --map-id" in context.cli_docs
    ), "CLI docs should include explore with map-id example"
    assert (
        "gather-explorer explore --all-maps" in context.cli_docs
    ), "CLI docs should include explore with all-maps example"
    assert (
        "gather-explorer validate" in context.cli_docs
    ), "CLI docs should include validate example"
...
def step_then_see_command_options(context):
    ...
    assert (
        "Options:" in context.cli_docs
    ), "CLI docs should include Options section"
    assert (
        "--format" in context.cli_docs
    ), "CLI docs should explain format option"
    assert (
        "--output" in context.cli_docs
    ), "CLI docs should explain output option"
    assert (
        "--map-id" in context.cli_docs
    ), "CLI docs should explain map-id option"
    assert (
        "--all-maps" in context.cli_docs
    ), "CLI docs should explain all-maps option"
...
def step_then_see_expected_output_formats(context):
    ...
    assert (
        "Output format" in context.cli_docs or "format" in context.cli_docs
    ), "CLI docs should mention output formats"
...
def step_then_see_public_classes_and_methods(context):
    ...
    assert (
        "Classes" in context.api_docs
    ), "API docs should include Classes section"
    assert (
        "GatherClient" in context.api_docs
    ), "API docs should include GatherClient class"
    assert (
        "PortalExplorer" in context.api_docs
    ), "API docs should include PortalExplorer class"
    assert (
        "Methods" in context.api_docs
    ), "API docs should include Methods section"
    assert (
        "get_space" in context.api_docs
    ), "API docs should include get_space method"
    assert (
        "get_maps" in context.api_docs
    ), "API docs should include get_maps method"
    assert (
        "analyze_map_portals" in context.api_docs
    ), "API docs should include analyze_map_portals method"
...
def step_then_see_parameter_descriptions(context):
    ...
    assert (
        "api_key" in context.api_docs
    ), "API docs should mention api_key parameter"
    assert (
        "space_id" in context.api_docs
    ), "API docs should mention space_id parameter"
    assert (
        "map_id" in context.api_docs
    ), "API docs should mention map_id parameter"
    assert (
        "output_file" in context.api_docs
    ), "API docs should mention output_file parameter"
...
def step_then_see_method_usage_examples(context):
    ...
    assert (
        "client = GatherClient" in context.api_docs
    ), "API docs should show client initialization"
    assert (
        "explorer = PortalExplorer" in context.api_docs
    ), "API docs should show explorer initialization"
...
def step_then_see_common_use_case_examples(context):
    ...
    assert (
        "list_maps_example.py" in context.example_scripts
    ), "Examples should include map listing"
    assert (
        "explore_portals_example.py" in context.example_scripts
    ), "Examples should include portal exploration"
...
def step_then_examples_are_well_commented(context):
    ...
    for script_name, script_content in context.example_scripts.items():
        ...
        assert (
            "Example:" in script_content
        ), f"{script_name} should include an example description"
...
def step_then_examples_runnable_with_minimal_config(context):
    ...
    for script_name, script_content in context.example_scripts.items():
        assert (
            "YOUR_API_KEY" in script_content
        ), f"{script_name} should include API key placeholder"
        assert (
            "YOUR_SPACE_ID" in script_content
        ), f"{script_name} should include space ID placeholder"
...
def step_then_examples_demonstrate_key_features(context):
    ...
    has_client_example = any(
        all(feature in script for feature in client_features)
    ...
    has_explorer_example = any(
        all(feature in script for feature in explorer_features)
        for script in context.example_scripts.values()
    )
    assert (
        has_explorer_example
    ), "Examples should demonstrate explorer features"

    # Check formatting features are demonstrated
    has_formatting_example = any(
        any(feature in script for feature in formatting_features)
        for script in context.example_scripts.values()
    )
    assert (
        has_formatting_example
    ), "Examples should demonstrate formatting features"
...
def step_then_see_interactive_portal_analysis(context):
    ...
    assert (
        "portal_analysis.ipynb" in context.notebook_examples
    ), "Notebooks should include portal analysis"
...
def step_then_see_portal_connection_visualizations(context):
    ...
    assert (
        "connection_visualization.ipynb" in context.notebook_examples
    ), "Notebooks should include connection visualization"
...
def step_then_see_common_error_scenarios(context):
    ...
    assert (
        "Common Error Scenarios" in context.error_docs
    ), "Error docs should include common scenarios section"
    assert (
        "Authentication Errors" in context.error_docs
    ), "Error docs should include authentication errors"
    assert (
        "Connection Errors" in context.error_docs
    ), "Error docs should include connection errors"
...
def step_then_see_troubleshooting_steps(context):
    ...
    assert (
        "Resolution:" in context.error_docs
    ), "Error docs should include resolution steps"
    assert (
        "Verify your API key" in context.error_docs
    ), "Error docs should include API key verification steps"
    assert (
        "Check your internet connection" in context.error_docs
    ), "Error docs should include connection troubleshooting"
...
def step_then_see_error_message_examples(context):
    ...
    assert (
        "Error: Unable to authenticate" in context.error_docs
    ), "Error docs should include authentication error example"
    assert (
        "Error: Unable to connect" in context.error_docs
    ), "Error docs should include connection error example"
...
def step_then_see_api_auth_resolution(context):
    ...
    assert (
        "Authentication Errors" in context.error_docs
    ), "Error docs should include authentication errors section"
    assert (
        "Verify your API key" in context.error_docs
    ), "Error docs should mention verifying API key"
    assert (
        "environment variables" in context.error_docs
    ), "Error docs should mention environment variables"
...
def step_then_receive_space_details(context):
    ...
    assert (
        context.space_result["id"] == "test_space_id"
    ), "Space details should include ID"
    assert (
        context.space_result["name"] == "Test Space"
    ), "Space details should include name"
...
def step_then_receive_objects_list(context):
    ...
    assert (
        context.objects_result is not None
    ), "Should receive a list of objects"
...
def step_then_receive_portal_objects(context):
    ...
    assert (
        context.portals_result is not None
    ), "Should receive a list of portals"
    assert (
        len(context.portals_result) > 0
    ), "Should receive at least one portal"
