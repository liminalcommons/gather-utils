...
def step_given_have_portal_model_instance(context):
    ...
    class MockPortal:
        ...
        def is_valid(self):
            ...
            return (self.properties.target_map is not None and
...
def step_then_portal_has_correct_attributes(context):
    ...
    assert context.portal.id == context.portal_data["id"], "Portal ID should match"
    assert context.portal.type == context.portal_data["type"], "Portal type should match"
    assert context.portal.x == context.portal_data["x"], "Portal X coordinate should match"
    assert context.portal.y == context.portal_data["y"], "Portal Y coordinate should match"
    
    assert context.portal.properties.target_map == context.portal_data["properties"]["targetMap"], "Target map should match"
    assert context.portal.properties.target_x == context.portal_data["properties"]["targetX"], "Target X should match"
    assert context.portal.properties.target_y == context.portal_data["properties"]["targetY"], "Target Y should match"
    assert context.portal.properties.normal == context.portal_data["properties"]["normal"], "Normal property should match"
...
def step_then_return_false_for_invalid_portals(context):
    ...
    assert context.invalid_result is False, "Invalid portal should return False"
...
def step_then_return_correct_destination(context):
    ...
    assert context.destination["map_id"] == context.portal_data["properties"]["targetMap"], "Destination map should match"
    assert context.destination["x"] == context.portal_data["properties"]["targetX"], "Destination X should match"
    assert context.destination["y"] == context.portal_data["properties"]["targetY"], "Destination Y should match"
...
def step_then_dict_contains_all_information(context):
    ...
    assert context.portal_dict["id"] == context.portal_data["id"], "Dictionary ID should match"
    assert context.portal_dict["type"] == context.portal_data["type"], "Dictionary type should match"
    assert context.portal_dict["x"] == context.portal_data["x"], "Dictionary X should match"
    assert context.portal_dict["y"] == context.portal_data["y"], "Dictionary Y should match"
    assert context.portal_dict["target_map"] == context.portal_data["properties"]["targetMap"], "Dictionary target map should match"
    assert context.portal_dict["target_x"] == context.portal_data["properties"]["targetX"], "Dictionary target X should match"
    assert context.portal_dict["target_y"] == context.portal_data["properties"]["targetY"], "Dictionary target Y should match"
    assert context.portal_dict["is_valid"] is True, "Dictionary is_valid should be True"
...
def step_then_portals_filtered_from_other_types(context):
    ...
    for portal in context.portals:
        assert portal["type"] == "portal", "Only portal objects should be returned"
...
def step_then_correctly_identify_all_portals(context):
    ...
    assert context.script_results["portals_found"] > 0, "Script should find portals"
    assert len(context.script_results["portals"]) == context.script_results["portals_found"], "Portal count should match"
...
def step_then_parse_portal_properties_accurately(context):
    ...
    for portal in context.script_results["portals"]:
        assert "properties" in portal, "Portal should have properties"
        assert "targetMap" in portal["properties"], "Portal should have targetMap property"
        assert "targetX" in portal["properties"], "Portal should have targetX property"
        assert "targetY" in portal["properties"], "Portal should have targetY property"
...
def step_then_handle_different_portal_configurations(context):
    ...
    assert has_normal, "Script should handle portals with different property sets"
...
def step_then_identify_bidirectional_portals(context):
    ...
    assert "bidirectional" in context.connection_analysis, "Connection analysis should identify bidirectional portals"
    assert len(context.connection_analysis["bidirectional"]) > 0, "Connection analysis should find at least one bidirectional portal pair"
...
def step_then_create_connection_graph(context):
    ...
    assert len(maps_connected) >= 2, "Connection graph should include at least two maps"
...
def step_then_detect_orphaned_or_one_way_portals(context):
    ...
    assert "one_way" in context.connection_analysis, "Connection analysis should detect one-way portals"
    assert "orphaned" in context.connection_analysis, "Connection analysis should detect orphaned portals"
...
def step_then_output_has_structured_format(context):
    ...
    assert context.save_result["format"] == "json", "Output should be in JSON format"
...
def step_then_include_all_portal_information(context):
    ...
    assert context.save_result["size_bytes"] > 0, "Output file should not be empty"
...
def step_then_tests_cover_edge_cases(context):
    ...
    assert "test_portal_validation" in context.test_results["results"], "Tests should include validation tests, which cover edge cases"
...
def step_then_tests_verify_portal_property_validation(context):
    ...
    assert "test_portal_validation" in context.test_results["results"], "Tests should verify portal property validation"
    assert context.test_results["results"]["test_portal_validation"] == "PASS", "Portal validation test should pass"
