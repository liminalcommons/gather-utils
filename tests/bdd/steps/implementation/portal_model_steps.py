...
def step_when_check_portal_validity(context):
    ...
    invalid_portal_data["properties"] = invalid_portal_data["properties"].copy()
...
def step_then_portal_has_correct_attributes(context):
    ...
    assert context.portal.properties.target_map == "map2", "Portal should have correct target map"
    assert context.portal.properties.target_x == 10, "Portal should have correct target X"
    assert context.portal.properties.target_y == 20, "Portal should have correct target Y"
    assert context.portal.properties.normal is True, "Portal should have correct normal property"
...
def step_then_return_correct_destination(context):
    ...
    assert context.destination["map_id"] == "map2", "Destination should have correct map ID"
    assert context.destination["x"] == 10, "Destination should have correct X coordinate"
    assert context.destination["y"] == 20, "Destination should have correct Y coordinate"
...
def step_then_dict_contains_all_info(context):
    ...
    assert context.portal_dict["id"] == "portal1", "Dictionary should include ID"
    assert context.portal_dict["type"] == 4, "Dictionary should include type"
    assert context.portal_dict["x"] == 5, "Dictionary should include X coordinate"
    assert context.portal_dict["y"] == 5, "Dictionary should include Y coordinate"
    assert context.portal_dict["target_map"] == "map2", "Dictionary should include target map"
    assert context.portal_dict["target_x"] == 10, "Dictionary should include target X"
    assert context.portal_dict["target_y"] == 20, "Dictionary should include target Y"
    assert context.portal_dict["is_valid"] is True, "Dictionary should include validity"
...
def step_then_parse_portal_properties_accurately(context):
    ...
    assert portal["properties"]["targetMap"] == "map2", "Target map should be parsed correctly"
    assert portal["properties"]["targetX"] == 10, "Target X should be parsed correctly"
    assert portal["properties"]["targetY"] == 20, "Target Y should be parsed correctly"
...
def step_then_create_connection_graph(context):
    ...
    assert "bidirectional" in context.connection_analysis, "Analysis should include bidirectional connections"
    assert "one_way" in context.connection_analysis, "Analysis should include one-way connections"
    assert "orphaned" in context.connection_analysis, "Analysis should include orphaned portals"
...
def step_then_output_has_structured_format(context):
    ...
    assert context.save_result["success"] is True, "Save operation should be successful"
    assert context.save_result["file"] == context.output_file, "Save result should include the correct filename"
