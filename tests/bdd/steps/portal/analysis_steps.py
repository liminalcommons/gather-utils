...
@when(
    'I run the command "gather-manager validate-portals --check-bidirectional"'
)
...
def step_then_see_fixed_issues_list(context):
    ...
    assert (
        "Issues Fixed" in context.result["output"]
    ), "Output should include issues fixed section"

    # Check for specific issues being fixed
    fixed_issue_indicators = ["Added", "Fixed", "Corrected"]
    has_fixed_issue = any(
        indicator in context.result["output"]
        for indicator in fixed_issue_indicators
    )
...
def step_then_see_manual_intervention_issues(context):
    ...
    assert context.result is not None, "Result should be defined"
    assert (
        "Manual Intervention" in context.result["output"]
    ), "Output should include manual intervention section"
    ...
    has_manual_issue = any(
        indicator in context.result["output"]
        for indicator in manual_issue_indicators
    )
    assert (
        has_manual_issue
    ), "Output should describe issues needing manual intervention"
...
def step_then_changes_applied_to_space(context):
    ...
    assert context.result is not None, "Result should be defined"
    assert (
        "Changes have been applied" in context.result["output"]
    ), "Output should confirm changes were applied"
...
def step_then_see_portal_health_summary(context):
    ...
    assert context.result is not None, "Result should be defined"
    assert (
        "Portal Health Report" in context.result["output"]
    ), "Output should include portal health report"
    assert (
        "Summary" in context.result["output"]
...
def step_then_see_portal_issue_statistics(context):
    ...
    assert context.result is not None, "Result should be defined"
    assert (
        "Issues by Type" in context.result["output"]
    ), "Output should include issues by type section"

    # Check for issue types and percentages
    issue_type_indicators = ["Missing", "%"]
    has_issue_types = all(
        indicator in context.result["output"]
        for indicator in issue_type_indicators
    )
...
def step_then_see_maps_with_most_issues(context):
    ...
    assert context.result is not None, "Result should be defined"
    assert (
        "Maps with Issues" in context.result["output"]
    ), "Output should include maps with issues section"

    # Check for map-specific issue counts
    map_issue_indicators = ["map", "issues"]
    has_map_issues = all(
        indicator in context.result["output"]
        for indicator in map_issue_indicators
    )
...
def step_then_report_saved_to_file(context):
    ...
    assert (
        ...
        or "Report saved" in context.result["output"]
    ), "Output should confirm report was saved"
    ...
    has_report_file = any(
        indicator in context.result["output"]
        for indicator in report_file_indicators
    )
...
def step_then_see_map_specific_property_analysis(context):
    ...
    map_specific_indicators = [context.map_id, "Map", "in map"]
    has_map_specific = any(
        indicator in context.result["output"]
        for indicator in map_specific_indicators
    )
