...
def step_then_provide_standardized_documentation(context):
    ...
    assert context.repository_docs["exists"], "Repository documentation should exist"
    assert "Repository Structure" in context.repository_docs["content"], "Documentation should have a clear title"
    assert "Directory Organization" in context.repository_docs["content"], "Documentation should include directory organization"
...
def step_then_explain_component_purpose(context):
    ...
    content = context.repository_docs["content"]
    assert "Source code" in content, "Documentation should explain source code purpose"
    assert "Test suite" in content, "Documentation should explain test suite purpose"
    assert "Documentation" in content, "Documentation should explain documentation purpose"
    assert "Utility scripts" in content, "Documentation should explain utility scripts purpose"
...
def step_then_confirm_free_of_critical_redundancies(context):
    ...
    has_critical = any(
        r for r in context.health_check_results["redundancies"] 
        if r["severity"] == "Critical"
    )
    assert not has_critical, "There should be no critical redundancies"
    assert not context.health_check_results["priorities"][0]["redundancies"], "There should be no high priority redundancies"
...
def step_then_redundancies_categorized_by_impact(context):
    ...
    assert len(context.health_check_results["priorities"]) > 0, "Redundancies should be categorized by priority"
    for priority in context.health_check_results["priorities"]:
        assert "priority" in priority, "Each category should have a priority level"
        assert "redundancies" in priority, "Each category should list redundancies"
...
def step_then_prioritized_resolution_plan(context):
    ...
    for redundancy in context.health_check_results["redundancies"]:
        assert "suggestion" in redundancy, "Each redundancy should have a resolution suggestion"
        assert redundancy["suggestion"], "Resolution suggestion should not be empty"
...
def step_then_accurately_reflect_components_and_relationships(context):
    ...
    assert "Component Relationships" in content, "Map should include component relationships"
    
    # Check for all main components
    components = ["api", "models", "services", "cli", "utils"]
    for component in components:
        assert component in content.lower(), f"Map should include {component} component"
    
    # Check for relationships (arrows or connections)
    assert "→" in content or "->" in content or "←" in content or "<-" in content, "Map should show relationships between components"
...
def step_then_identify_key_navigation_points(context):
    ...
    content = context.system_map["content"]
    assert "Navigation Points" in content, "Map should include navigation points section"
...
def step_then_match_actual_repository_structure(context):
    ...
    content = context.system_map["content"]
    assert "src/gather_manager/" in content, "Map should match the actual repository structure"
    assert "api/" in content and "models/" in content and "services/" in content, "Map should include all major components"
...
def step_then_testing_components_properly_organized(context):
    ...
    assert context.testing_structure["exists"], "Testing directory structure should exist"
    
    # Check for key directories
    expected_directories = ["tests/unit", "tests/integration", "tests/bdd"]
    for directory in expected_directories:
        assert directory in context.testing_structure["directories"], f"Testing structure should include {directory} directory"
...
def step_then_no_duplicated_test_functionality(context):
    ...
    for pattern in duplicate_patterns:
        assert pattern["count"] <= 1, f"There should be no duplicated {pattern['pattern']} test files"
...
def step_then_documentation_centrally_accessible(context):
    ...
    assert context.documentation_structure["exists"], "Documentation structure should exist"
    
    # Check for key directories
    expected_directories = ["docs/project", "docs/api", "docs/user_guide"]
    for directory in expected_directories:
        assert directory in context.documentation_structure["directories"], f"Documentation should include {directory} directory"
...
def step_then_documentation_follow_standards(context):
    ...
    for file in expected_files:
        assert file in context.documentation_structure["files"], f"Documentation should include {file}"
...
def step_then_required_documentation_present(context):
    ...
    for category in required_categories:
        ...
        assert dir_path in context.documentation_structure["directories"], f"Documentation should include {dir_path} directory"
        
        # Check that each directory has at least one file
        category_files = [f for f in context.documentation_structure["files"] if f.startswith(dir_path)]
        assert len(category_files) > 0, f"{dir_path} should contain documentation files"
...
def step_then_document_all_development_tools(context):
    ...
    assert context.tool_documentation["exists"], "Tool documentation should exist"
    assert len(context.tool_documentation["tools"]) > 0, "Documentation should include tool descriptions"
    ...
    for tool in context.tool_documentation["tools"]:
        ...
        assert "description" in tool, "Tool documentation should include tool description"
        assert "examples" in tool, "Tool documentation should include tool examples"
...
def step_then_include_complete_usage_examples(context):
    ...
    for tool in context.tool_documentation["tools"]:
        ...
        assert len(tool["examples"]) > 1, "Tool documentation should include multiple examples"
        
        # Check that examples show different usage patterns
        assert len(set(tool["examples"])) == len(tool["examples"]), "Examples should demonstrate different usages"
...
def step_then_instructions_clear_and_standardized(context):
    ...
    for tool in context.tool_documentation["tools"]:
        ...
        for example in tool["examples"]:
            assert example.startswith("python tools/"), "Examples should use standardized command format"
...
def step_then_hooks_detect_and_block_issues(context):
    ...
    assert not context.pre_commit_run["success"], "Pre-commit hooks should fail on issues"
    assert len(context.pre_commit_run["hooks"]) > 0, "Pre-commit hooks should detect issues"
    
    # Check that at least one hook failed
    failing_hooks = [h for h in context.pre_commit_run["hooks"] if h["status"] == "Failed"]
...
def step_then_helpful_error_messages(context):
    ...
    assert len(context.pre_commit_run["error_messages"]) > 0, "Pre-commit should provide error messages"
    ...
    for message in context.pre_commit_run["error_messages"]:
        ...
        assert "Run" in message or "fix" in message.lower() or "shorten" in message.lower(), "Error messages should include resolution guidance"
...
def step_then_verify_critical_aspects(context):
    ...
    assert context.health_check_execution["success"], "Health checks should execute successfully"
    
    # Check for critical health aspects
    critical_aspects = ["Directory Structure", "Code Style", "Test Coverage"]
    check_names = [check["name"] for check in context.health_check_execution["checks"]]
...
def step_then_reports_provide_actionable_insights(context):
    ...
    for check in context.health_check_execution["checks"]:
        ...
        if check["status"] != "PASS":
            assert "%" in check["message"] or ":" in check["message"], "Non-passing checks should provide specific information"
...
def step_then_health_checks_documented(context):
    ...
    assert "documentation" in context.health_check_execution, "Health checks should have documentation"
    assert context.health_check_execution["documentation"], "Documentation reference should not be empty"
...
def step_then_provide_access_to_required_tasks(context):
    ...
    assert context.dev_cli_usage["success"], "Development CLI should function successfully"
    
    # Check for key development tasks
    required_tasks = ["setup", "test", "lint"]
    command_names = [cmd["command"].split()[1] for cmd in context.dev_cli_usage["commands_used"]]
    
    for task in required_tasks:
        assert task in command_names, f"CLI should provide access to {task} task"
...
def step_then_commands_work_as_documented(context):
    ...
    for command in context.dev_cli_usage["commands_used"]:
        ...
        assert "error" not in command["result"].lower(), "Command should not produce errors"
        assert "fail" not in command["result"].lower(), "Command should not fail"
...
def step_then_documentation_cover_all_commands(context):
    ...
    assert "documentation" in context.dev_cli_usage, "CLI should have documentation"
    assert context.dev_cli_usage["documentation"], "Documentation reference should not be empty"
...
def step_then_comply_with_file_structure_conventions(context):
    ...
    assert context.repository_structure["follows_conventions"], "Repository should follow file structure conventions"
    assert context.repository_structure["conventions_doc"], "Conventions document should exist"
...
def step_then_conventions_clearly_documented(context):
    ...
    assert context.repository_structure["conventions_doc"], "Conventions should be documented"
    assert "docs/project/file_conventions.md" in context.repository_structure["conventions_doc"], "Conventions should be in the docs directory"
...
def step_then_include_practical_examples(context):
    ...
    assert "structure" in context.repository_structure, "Repository structure should be documented"
    assert len(context.repository_structure["structure"]) > 0, "Structure documentation should not be empty"
...
def step_then_cover_agent_interaction_patterns(context):
    ...
    assert context.agent_documentation["exists"], "Agent documentation should exist"
    assert "Navigation Patterns" in context.agent_documentation["sections"], "Documentation should cover navigation patterns"
    assert "Decision Framework" in context.agent_documentation["sections"], "Documentation should cover decision framework"
...
def step_then_integrated_with_main_documentation(context):
    ...
    assert context.agent_documentation["integration"] == "Integrated with main documentation", "Agent documentation should be integrated with main documentation"
...
def step_then_provide_all_navigation_information(context):
    ...
    assert context.agent_documentation["thoroughness"] == "Comprehensive", "Agent documentation should be comprehensive"
    
    # Check for essential sections
    essential_sections = ["Navigation Patterns", "Essential Commands Reference"]
    for section in essential_sections:
        assert section in context.agent_documentation["sections"], f"Documentation should include {section} section"
...
def step_then_verify_compliance_with_standards(context):
    ...
    assert context.structure_validation["success"], "Structure validation should succeed"
    
    # Check for key validation points
    key_validations = ["Directory Structure", "File Naming", "Module Organization"]
    validation_names = [v["name"] for v in context.structure_validation["validation_points"]]
    
    for validation in key_validations:
        assert validation in validation_names, f"Validation should check {validation}"
...
def step_then_generate_accurate_validation_reports(context):
    ...
    assert "report" in context.structure_validation, "Validation should generate a report"
    assert context.structure_validation["report"], "Report reference should not be empty"
...
def step_then_highlight_compliance_issues(context):
    ...
    for point in context.structure_validation["validation_points"]:
        assert point["status"] in ["PASS", "WARNING", "FAIL"], "Validation point should have a clear status"
        assert point["message"], "Validation point should have a descriptive message"
...
def step_then_agents_navigate_key_parts(context):
    ...
    assert context.agent_navigation["success"], "Agent navigation tests should succeed"
    
    # Check that all navigation tasks succeeded
    for path in context.agent_navigation["navigation_paths"]:
        assert path["success"], f"Agent should successfully navigate to {path['task']}"
...
def step_then_navigation_paths_efficient(context):
    ...
    for path in context.agent_navigation["navigation_paths"]:
        assert path["steps"] <= 5, f"Navigation to {path['task']} should take 5 or fewer steps"
...
def step_then_navigation_reports_show_coverage(context):
    ...
    assert "coverage" in context.agent_navigation, "Navigation tests should report on coverage"
    assert "All key components" in context.agent_navigation["coverage"], "Coverage should include all key components"
    assert "report" in context.agent_navigation, "Navigation tests should generate a report"
