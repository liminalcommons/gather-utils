...
from tests.bdd.steps.system.development_verification_steps import step_given_have_existing_development_system, step_given_have_agent_and_project_systems
...
def step_given_have_established_release_standards(context):
    ...
    assert "definition_of_done" in context.release_standards, "Release standards should include definition of done"
    assert "release_process" in context.release_standards, "Release standards should include release process"
    assert "deliverables" in context.release_standards, "Release standards should include deliverables"
...
def step_given_release_completed(context, release_number):
    ...
    assert context.release_status["complete"], f"Release {release_number} should be complete"
...
def step_given_bdd_release_completed(context, release_number):
    ...
    # Assert that the release is complete
    assert context.release_status["complete"], f"BDD Release {release_number} should be complete"
...
def step_when_review_dod_documentation(context):
    ...
    assert "original" in context.dod_evolution, "DoD evolution should include original version"
    assert "updated" in context.dod_evolution, "DoD evolution should include updates"
    assert len(context.dod_evolution["updated"]) > 0, "DoD should have been updated at least once"
...
def step_when_evaluate_agent_bdd_support(context):
    ...
    assert "components" in context.agent_bdd_support, "Agent BDD support should include components"
    assert "effectiveness" in context.agent_bdd_support, "Agent BDD support should include effectiveness evaluation"
    assert len(context.agent_bdd_support["components"]) >= 5, "Should have at least 5 BDD support components"
...
def step_then_meet_all_dod_criteria(context):
    ...
    assert context.release_status["meets_dod"], f"Release {context.current_release} should meet Definition of Done"
...
def step_then_deliverables_present_and_complete(context):
    ...
    for row in context.table:
        ...
        assert deliverable_name in context.release_status["deliverables"], \
        ...
        assert deliverable["present"], f"Deliverable '{deliverable_name}' should be present"
        assert deliverable["complete"], f"Deliverable '{deliverable_name}' should be complete"
        
        # Check that the location matches
        assert deliverable["location"] == expected_location, \
...
def step_then_repository_structure_standardized(context):
    ...
    for deliverable in structure_related_deliverables:
        assert deliverable in context.release_status["deliverables"], \
            f"Structure-related deliverable '{deliverable}' should be present"
        assert context.release_status["deliverables"][deliverable]["present"], \
            f"Structure-related deliverable '{deliverable}' should be present"
        assert context.release_status["deliverables"][deliverable]["complete"], \
...
def step_then_show_progressive_refinement(context):
    ...
    previous_criteria_count = len(context.dod_evolution["original"]["criteria"])
    for update in context.dod_evolution["updated"]:
        current_criteria_count = len(update["criteria"])
        assert current_criteria_count >= previous_criteria_count, "DoD updates should add or refine criteria"
...
def step_then_incorporate_lessons_learned(context):
    ...
    for update in context.dod_evolution["updated"]:
        assert "lessons_learned" in update, "DoD updates should include lessons learned"
        assert len(update["lessons_learned"]) > 0, "DoD updates should have at least one lesson learned"
...
def step_then_included_in_cicd_verification(context):
    ...
    assert "ci_integration" in context.dod_evolution, "DoD should be integrated with CI/CD"
    assert context.dod_evolution["ci_integration"]["present"], "DoD CI/CD integration should be present"
    assert context.dod_evolution["ci_integration"]["failure_behavior"], "DoD CI/CD integration should define failure behavior"
...
def step_then_reflected_in_bdd_scenarios(context):
    ...
    assert "bdd_scenarios" in context.dod_evolution, "DoD should have BDD scenarios"
    assert context.dod_evolution["bdd_scenarios"]["present"], "DoD BDD scenarios should be present"
    assert context.dod_evolution["bdd_scenarios"]["scenario_count"] > 0, "DoD should have at least one BDD scenario"
...
def step_then_components_present_and_functional(context):
    ...
    for row in context.table:
        ...
        assert component_key in context.agent_bdd_support["components"], \
        ...
        assert component["present"], f"Component '{component_name}' should be present"
        assert component["complete"], f"Component '{component_name}' should be complete"
        
        # Check that the location matches
        assert component["location"] == expected_location, \
...
def step_then_agents_effectively_operate_bdd_system(context):
    ...
    assert context.agent_bdd_support["effectiveness"]["score"] >= 4.0, \
        "Agent BDD support effectiveness score should be at least 4.0"
    assert len(context.agent_bdd_support["effectiveness"]["successful_operations"]) >= 4, \
