...
def mock_portal_service():
    ...
    mock_service.validate_portals.return_value = {
        "valid_portals": [
            {"id": "portal1", "map_id": "map1", "x": 5, "y": 5, "target_map": "map2", "target_x": 10, "target_y": 20, "is_valid": True}
        ],
        "invalid_portals": [
            {"id": "portal2", "map_id": "map1", "x": 15, "y": 15, "target_map": None, "is_valid": False, "reason": "Missing target map"}
        ]
    }
...
class TestPortalCommands:
    ...
    def test_portal_validation_command(self, mock_portal_service_class, cli_runner, mock_portal_service):
        """Test the portal validation command."""
        # Setup the mock
        mock_portal_service_class.return_value = mock_portal_service
        
        # Run the command
        result = cli_runner.invoke(app, ["portals", "validate", "--space-id", "test_space_id"])
    ...
    @patch("gather_manager.cli.main.PortalService")
    def test_portal_connections_command(self, mock_portal_service_class, cli_runner, mock_portal_service):
        """Test the portal connections command."""
        # Setup the mock
        mock_portal_service_class.return_value = mock_portal_service
        
        # Run the command
        result = cli_runner.invoke(app, ["portals", "connections", "--space-id", "test_space_id"])
    ...
    @patch("gather_manager.cli.main.PortalService")
    def test_portal_details_command(self, mock_portal_service_class, cli_runner, mock_portal_service):
        """Test the portal details command."""
        # Setup the mock
        mock_portal_service_class.return_value = mock_portal_service
        
        # Run the command
        result = cli_runner.invoke(app, ["portals", "details", "--space-id", "test_space_id", "--map-id", "map1"])
    ...
    @patch("gather_manager.cli.main.PortalService")
    def test_portal_export_command_json(self, mock_portal_service_class, cli_runner, mock_portal_service):
        ...
        # Run the command
        result = cli_runner.invoke(app, ["portals", "export", "--space-id", "test_space_id", "--format", "json"])
        
        # Verify the command was successful
        assert result.exit_code == 0
        
        # Verify the service was called with the correct parameters
        mock_portal_service.export_portals.assert_called_once_with(format="json", output_dir="data")
    ...
    @patch("gather_manager.cli.main.PortalService")
    def test_portal_export_command_csv(self, mock_portal_service_class, cli_runner, mock_portal_service):
        ...
        # Run the command
        result = cli_runner.invoke(app, ["portals", "export", "--space-id", "test_space_id", "--format", "csv"])
        
        # Verify the command was successful
        assert result.exit_code == 0
        
        # Verify the service was called with the correct parameters
        mock_portal_service.export_portals.assert_called_once_with(format="csv", output_dir="data")
