...
def migrate_feature_files():
    ...
    for feature_file in feature_files:
        ...
        if target_file.exists():
            print(
                f"Skipping {feature_file.name} - already exists in target directory"
            )
            continue
...
def migrate_environment_files():
    ...
    if source_file.exists():
        ...
        if target_file.exists():
            print(
                f"Skipping environment.py - already exists in target directory"
            )
            return False
...
def migrate_step_definitions():
    ...
    for step_file in step_files:
        ...
        if target_file.exists():
            print(
                f"Skipping {step_file.name} - already exists in target directory"
            )
            continue
...
def main():
    ...
    print(
        f"Environment Files: {'MIGRATED' if environment_migrated else 'NO CHANGES'}"
    )
    print(
        f"Step Definitions: {'MIGRATED' if steps_migrated else 'NO CHANGES'}"
    )
    ...
    print(
        "You can now run your BDD tests using: python tools/run_bdd_tests.py"
    )
