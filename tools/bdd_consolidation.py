...
def run_phase(phase, args=None):
    ...
    try:
        ...
        if success:
            ...
            logger.error(f"Phase {phase} failed with exit code {result.returncode}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BDD Test Structure Consolidation")
    parser.add_argument("--start-phase", type=int, choices=range(1, 7), default=1,
                        help="Start execution from this phase (1-6, default: 1)")
    parser.add_argument("--end-phase", type=int, choices=range(1, 7), default=6,
    ...
    if start_phase > end_phase:
        logger.error(f"Start phase ({start_phase}) cannot be greater than end phase ({end_phase})")
        print(f"Error: Start phase ({start_phase}) cannot be greater than end phase ({end_phase})")
        return 1
    
    logger.info(f"Starting BDD Test Structure Consolidation (phases {start_phase}-{end_phase})")
    print(f"Starting BDD Test Structure Consolidation (phases {start_phase}-{end_phase})")
    ...
    for phase in range(start_phase, end_phase + 1):
        ...
        elif phase == 2 and start_phase != phase:  # Phase 2 called from full sequence
            phase_args = ["--cleanup"]  # Clean up in full sequence
        
        if phase == 3 and start_phase == phase:  # First run of Phase 3
            phase_args = []  # No arguments for first run
        elif phase == 3 and start_phase != phase:  # Phase 3 called from full sequence
            phase_args = ["--cleanup"]  # Clean up in full sequence
            
        # Execute the phase
        if not run_phase(phase, phase_args):
            logger.error(f"Stopping consolidation process due to failure in phase {phase}")
