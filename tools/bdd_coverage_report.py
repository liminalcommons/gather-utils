...
class BDDCoverageAnalyzer:
    ...
    def load_requirements(self):
        ...
        if self.requirements_file and os.path.exists(self.requirements_file):
            ...
            logger.warning("No requirements file provided or file not found. "
    ...
    def scan_feature_files(self):
        ...
        feature_files = glob.glob(os.path.join(self.features_dir, "**", "*.feature"), recursive=True)
    ...
    def _process_feature_file(self, file_path: str):
        ...
        relative_path = os.path.relpath(file_path, os.path.dirname(self.features_dir))
        ...
        # Extract scenarios
        scenarios = self._extract_scenarios(content, relative_path, feature_name, feature_tags)
    ...
    def _extract_scenarios(self, content: str, file_path: str, feature_name: str, feature_tags: List[str]) -> List[BDDScenario]:
        ...
        for scenario_match in scenario_blocks:
            ...
            tags_section = content[max(0, content.rfind('\n', 0, scenario_start)):scenario_start]
            ...
            steps_count = len(re.findall(r'^\s*(Given|When|Then|And|But)\s+', scenario_content, re.MULTILINE))
            ...
            scenario = BDDScenario(
                ...
                status=self._determine_scenario_status(scenario_name, file_path),
    ...
    def _determine_scenario_status(self, scenario_name: str, feature_file: str) -> str:
        """
    ...
    def find_coverage_gaps(self):
        ...
        for req_id, req in self.requirements.items():
            if req.scenarios_count == 0:
                ...
                self.coverage_gaps.append(CoverageGap(
                    ...
                    risk_level="High" if req.priority == "High" else "Medium"
                ))
        ...
        self.coverage_gaps.sort(
            key=lambda gap: (
                0 if gap.risk_level == "High" else (1 if gap.risk_level == "Medium" else 2),
    ...
    def generate_reports(self, output_dir: str):
        ...
        self._generate_summary_report(os.path.join(output_dir, "coverage_summary.md"))
        
        # Generate detailed report (JSON)
        self._generate_detailed_report(os.path.join(output_dir, "coverage_detailed.json"))
    ...
    def _generate_summary_report(self, output_file: str):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            covered_reqs = sum(1 for req in self.requirements.values() if req.scenarios_count > 0)
            fully_covered_reqs = sum(1 for req in self.requirements.values() if req.coverage_percentage == 100)
            
            f.write(f"- Total Requirements: {total_reqs}\n")
            f.write(f"- Requirements with BDD Coverage: {covered_reqs} ({covered_reqs/total_reqs*100:.1f}%)\n")
            f.write(f"- Fully Covered Requirements: {fully_covered_reqs} ({fully_covered_reqs/total_reqs*100:.1f}%)\n")
            f.write(f"- Total BDD Scenarios: {len(self.scenarios)}\n")
            f.write(f"- Coverage Gaps Identified: {len(self.coverage_gaps)}\n\n")
            
            # Coverage by priority
            f.write("## Coverage by Priority\n\n")
            f.write("| Priority | Total | Covered | Fully Covered | Coverage % |\n")
            f.write("|----------|-------|---------|---------------|------------|\n")
            
            for priority in ["High", "Medium", "Low"]:
                priority_reqs = [req for req in self.requirements.values() if req.priority == priority]
                total = len(priority_reqs)
                if total == 0:
                    continue
                
                covered = sum(1 for req in priority_reqs if req.scenarios_count > 0)
                fully_covered = sum(1 for req in priority_reqs if req.coverage_percentage == 100)
                coverage_pct = (fully_covered / total * 100) if total > 0 else 0
                
                f.write(f"| {priority} | {total} | {covered} | {fully_covered} | {coverage_pct:.1f}% |\n")
            ...
            f.write(f"| High | {risk_levels['High']} | High priority requirements with insufficient coverage |\n")
            f.write(f"| Medium | {risk_levels['Medium']} | Medium priority requirements with poor coverage or high priority with partial coverage |\n")
            f.write(f"| Low | {risk_levels['Low']} | Well-covered requirements |\n")
            
            # Top gaps
            if self.coverage_gaps:
                f.write("\n## Top Coverage Gaps\n\n")
                f.write("| Requirement | Priority | Gap Type | Risk Level | Recommendation |\n")
                f.write("|-------------|----------|----------|------------|----------------|\n")
                
                # Show top 10 gaps
                for gap in self.coverage_gaps[:10]:
                    f.write(f"| REQ-{gap.requirement_id} | {gap.priority} | {gap.gap_type} | {gap.risk_level} | {gap.recommendation} |\n")
            ...
            high_risk_gaps = [gap for gap in self.coverage_gaps if gap.risk_level == "High"]
            if high_risk_gaps:
            ...
            f.write("\n### General Recommendations\n\n")
            no_coverage = sum(1 for gap in self.coverage_gaps if gap.gap_type == "No Coverage")
            partial_coverage = sum(1 for gap in self.coverage_gaps if gap.gap_type == "Partial Coverage")
            failed_tests = sum(1 for gap in self.coverage_gaps if gap.gap_type == "Failed Tests")
            
            if no_coverage > 0:
                f.write(f"1. Create BDD scenarios for {no_coverage} requirements with no coverage\n")
            if partial_coverage > 0:
                f.write(f"2. Improve scenario coverage for {partial_coverage} partially covered requirements\n")
            if failed_tests > 0:
                f.write(f"3. Fix {failed_tests} failing BDD scenarios\n")
            f.write("4. Regularly review and update the BDD coverage analysis\n")
            f.write("5. Consider automating the coverage reporting in the CI/CD pipeline\n")
    ...
    def _generate_gap_report(self, output_file: str):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            if "No Coverage" in gaps_by_type:
                ...
                f.write("| Requirement | Description | Priority | Risk Level |\n")
                f.write("|-------------|-------------|----------|------------|\n")
                
                for gap in gaps_by_type["No Coverage"]:
                    f.write(f"| REQ-{gap.requirement_id} | {gap.requirement_description} | {gap.priority} | {gap.risk_level} |\n")
                
                f.write("\n### Recommendations\n\n")
                f.write("1. Prioritize creating BDD scenarios for high priority requirements\n")
                f.write("2. Create at least one scenario for each requirement to establish basic coverage\n")
                f.write("3. Focus on defining clear acceptance criteria in each scenario\n\n")
            
            # Partial Coverage section
            if "Partial Coverage" in gaps_by_type:
                f.write("## Requirements With Partial Coverage\n\n")
                f.write("| Requirement | Description | Priority | Current Coverage | Risk Level |\n")
                f.write("|-------------|-------------|----------|-----------------|--------------|\n")
                
                for gap in gaps_by_type["Partial Coverage"]:
                    req = self.requirements.get(gap.requirement_id)
                    coverage = f"{req.coverage_percentage:.1f}%" if req else "Unknown"
                    f.write(f"| REQ-{gap.requirement_id} | {gap.requirement_description} | {gap.priority} | {coverage} | {gap.risk_level} |\n")
                
                f.write("\n### Recommendations\n\n")
                f.write("1. Review existing scenarios for completeness\n")
                f.write("2. Add scenarios to cover additional aspects of these requirements\n")
                f.write("3. Focus on edge cases and negative testing scenarios\n\n")
            
            # Failed Tests section
            if "Failed Tests" in gaps_by_type:
                f.write("## Requirements With Failing Tests\n\n")
                f.write("| Requirement | Description | Priority | Failed / Total Scenarios | Risk Level |\n")
                f.write("|-------------|-------------|----------|--------------------------|------------|\n")
                
                for gap in gaps_by_type["Failed Tests"]:
                    req = self.requirements.get(gap.requirement_id)
                    ratio = f"{req.failed_scenarios}/{req.scenarios_count}" if req else "Unknown"
                    f.write(f"| REQ-{gap.requirement_id} | {gap.requirement_description} | {gap.priority} | {ratio} | {gap.risk_level} |\n")
                
                f.write("\n### Recommendations\n\n")
                f.write("1. Investigate and fix failing scenarios\n")
                f.write("2. Consider if failures indicate issues with the requirements\n")
                f.write("3. Ensure test environment is properly configured\n\n")
            ...
            medium_risk_gaps = [gap for gap in self.coverage_gaps if gap.risk_level == "Medium"]
            if medium_risk_gaps:
                ...
                if len(medium_risk_gaps) > 10:
                    f.write(f"...plus {len(medium_risk_gaps) - 10} more medium risk items\n")
                f.write("\n")
            
            # Low priority actions
            low_risk_gaps = [gap for gap in self.coverage_gaps if gap.risk_level == "Low"]
            if low_risk_gaps:
                f.write("### Long-term Actions (Low Risk)\n\n")
                f.write(f"There are {len(low_risk_gaps)} low risk coverage items that can be addressed as part of regular maintenance.\n")

def main():
    """Main entry point for the script."""
    parser = argparse.ArgumentParser(description='Generate a BDD coverage report')
    parser.add_argument('--features-dir', required=True, help='Directory containing BDD feature files')
    parser.add_argument('--requirements-file', help='File containing requirements data (CSV or JSON)')
