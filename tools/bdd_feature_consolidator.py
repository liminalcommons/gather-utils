"""
Tool: Bdd Feature Consolidator
Created: 2025-03-21
Author: Development Team
Status: Active
Purpose: Consolidate similar or duplicate BDD features
Dependencies: logging, shutil, tempfile, dataclasses
Lifecycle:
...
class Scenario:
    ...
    def component_tags(self) -> List[str]:
        ...
        return [
            ...
            if not tag.startswith("@REQ-") and not tag.startswith("@REL-")
        ]
...
class ConsolidationPlan:
    ...
    def _execute_merge(self) -> bool:
        ...
        logger.info(
            f"Merged {len(self.source_scenarios)} scenarios into {self.consolidated_scenario.name}"
        )
        return True
    ...
    def _execute_move(self) -> bool:
        ...
        logger.info(
            f"Moved scenario {scenario.name} to {self.target_feature.path}"
        )
        return True
    ...
    def _execute_create(self) -> bool:
        ...
        logger.info(
            f"Created new scenario {self.consolidated_scenario.name} in {self.target_feature.path}"
        )
        return True
    ...
    def _remove_scenario_from_feature(self, scenario: Scenario) -> bool:
        ...
        if feature.remove_scenario(scenario):
            ...
            logger.info(
                f"Removed scenario {scenario.name} from {feature_path}"
            )
            return True
        else:
            logger.warning(
                f"Failed to remove scenario {scenario.name} from {feature_path}"
            )
            return False
...
class FeatureConsolidator:
    ...
    def create_consolidation_plans(self):
        ...
        for duplicate in self.duplicates:
            ...
            if not source_feature or not target_feature:
                logger.warning(
                    f"Could not find features for {source_file} or {target_file}"
                )
                continue
            ...
            source_scenarios = [
                s
                for s in source_feature.scenarios
                if s.name == duplicate["name"]
            ]
            target_scenarios = [
                s
                for s in target_feature.scenarios
                if s.name == duplicate["name"]
            ]

            if not source_scenarios or not target_scenarios:
                logger.warning(
                    f"Could not find scenarios for duplicate {duplicate['name']}"
                )
                continue
    ...
    def _create_plan_for_duplicate(
        ...
        if similarity >= 95:  # Almost identical
            ...
            if target_scenario:
                # Merge tags
                merged_tags = list(
                    set(source_scenario.tags + target_scenario.tags)
                )
            ...
            logger.info(
                f"Similarity too low ({similarity}%) for automatic consolidation of {source_scenario.name}"
            )
            return None
    ...
    def execute_plans(self):
        ...
        logger.info(
            f"Executing {len(self.consolidation_plans)} consolidation plans"
        )

        success_count = 0
        for i, plan in enumerate(self.consolidation_plans):
            logger.info(
                f"Executing plan {i+1}/{len(self.consolidation_plans)}"
            )
            if plan.execute(dry_run=self.dry_run):
                success_count += 1

        logger.info(
            f"Successfully executed {success_count}/{len(self.consolidation_plans)} plans"
        )
    ...
    def generate_report(self, output_file: str):
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- Consolidation plans created: {len(self.consolidation_plans)}\n"
            )

            # Count by action
            action_counts = {}
            for plan in self.consolidation_plans:
                action_counts[plan.action] = (
                    action_counts.get(plan.action, 0) + 1
                )
            ...
            for i, plan in enumerate(self.consolidation_plans):
                ...
                if plan.target_scenario:
                    f.write(
                        f"- Target scenario: {plan.target_scenario.name}\n"
                    )
                ...
                if plan.consolidated_scenario:
                    f.write(
                        f"- Consolidated scenario: {plan.consolidated_scenario.name}\n"
                    )
                    f.write(
                        f"- Tags: {', '.join(plan.consolidated_scenario.tags)}\n"
                    )
...
def parse_feature_file(file_path: str) -> Optional[Feature]:
    ...
    description = (
        content[description_start:description_end].strip().split("\n")
    )
    ...
    for tags_match, scenario_match in scenario_pattern.findall(content):
        ...
        scenario_name_match = re.match(
            r"Scenario(?:\s+Outline)?:\s*(.+)$", scenario_content, re.MULTILINE
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Consolidate duplicate BDD scenarios"
    )
    parser.add_argument(
        "--features-dir",
        required=True,
    ...
    parser.add_argument(
        "--duplicates-report",
        required=True,
        help="Path to duplicates report from bdd_duplication_analyzer.py",
    )
    parser.add_argument(
        "--output-dir",
