...
def generate_feature_file(milestone, requirements):
    ...
    milestone_reqs = [req for req in requirements if req['milestone'] == milestone]
...
def extract_existing_step_definitions(steps_dir):
    ...
    for py_file in Path(steps_dir).glob('*.py'):
        with open(py_file, 'r') as f:
            ...
            step_patterns = re.findall(r'@(?:given|when|then)\([\'"](.+?)[\'"]\)\s*\ndef\s+(\w+)', content, re.IGNORECASE)
...
def generate_step_definition_file(milestone, steps, suggestions, output_dir):
    ...
    for step in steps:
        ...
        func_name = f"step_{step_text.lower().replace(' ', '_').replace('-', '_')}"
        ...
        if step in suggestions:
            ...
            content.append(f"    \"\"\"Implementation based on similar step: {suggested_step}\"\"\"")
            content.append(f"    # TODO: Adapt the implementation from {suggested_func}")
            content.append(f"    pass")
        else:
            content.append(f"    \"\"\"Implement the step definition for: {step}\"\"\"")
...
def extract_steps_from_feature(feature_content):
    ...
    for line in feature_content.split('\n'):
        line = line.strip()
        if line.startswith('Given ') or line.startswith('When ') or line.startswith('Then ') or line.startswith('And '):
            steps.append(line)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate BDD scenarios for missing requirements')
    parser.add_argument('--milestone', help='Generate scenarios only for the specified milestone')
    ...
    if args.milestone:
        ...
        if args.milestone in reqs_by_milestone:
            feature_content = generate_feature_file(args.milestone, requirements)
            if feature_content:
                file_path = save_feature_file(args.milestone, feature_content, output_dir)
                if file_path:
                    ...
                    suggestions = suggest_step_definitions(steps, existing_steps)
                    step_file = generate_step_definition_file(args.milestone, steps, suggestions, steps_dir)
                    if step_file:
                        generated_files.append((f"{args.milestone} Steps", step_file))
        else:
        ...
        for milestone in reqs_by_milestone:
            ...
            if feature_content:
                file_path = save_feature_file(milestone, feature_content, output_dir)
                if file_path:
                    ...
                    steps = extract_steps_from_feature(feature_content)
                    suggestions = suggest_step_definitions(steps, existing_steps)
                    step_file = generate_step_definition_file(milestone, steps, suggestions, steps_dir)
                    if step_file:
                        generated_files.append((f"{milestone} Steps", step_file))
