...
class BDDTestRunner:
        ...
        for feature in features:
            ...
            for line in content.split("\n"):
                if line.strip().startswith("@"):
                    feature_tags.update(tag.strip() for tag in line.strip().split("@")[1:])
            
            # Check if feature should be included
            if self.tags and not any(tag in feature_tags for tag in self.tags):
                continue
            if self.exclude_tags and any(tag in feature_tags for tag in self.exclude_tags):
                continue
            ...
            if json_output.exists():
                ...
                for scenario in feature_results.get("elements", []):
                    ...
                    results.append(TestResult(
                        ...
                        duration=duration,
                        error_message=None,
                        stdout=process.stdout if self.capture_stdout else None,
                        stderr=process.stderr if self.capture_stdout else None,
    ...
    def run_tests(self) -> None:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for future in as_completed(future_to_feature):
                    ...
                    try:
                        ...
                        # Check for fail fast
                        if self.fail_fast and any(r.status == "failed" for r in results):
        ...
        if self.rerun_failed and any(r.status == "failed" for r in self.results):
    ...
    def calculate_performance_metrics(self, total_duration: float) -> None:
        ...
        scenario_durations = [(r.feature + ":" + r.scenario, r.duration) 
                            for r in self.results]
        avg_duration = sum(d for _, d in scenario_durations) / len(scenario_durations)
    ...
    def _print_summary(self, summary: Dict) -> None:
        ...
        if self.performance:
            ...
            print(f"Average Scenario Duration: {self.performance.avg_scenario_duration:.2f}s")
            print("\nSlowest Scenarios:")
...
def main():
    ...
    parser.add_argument("--tags", type=str, nargs="*",
                      help="Only run features with these tags")
    parser.add_argument("--exclude-tags", type=str, nargs="*",
