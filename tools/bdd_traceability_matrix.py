...
class TraceabilityMatrixGenerator:
    ...
    def _process_feature_file(self, file_path: str):
        ...
        # Extract scenarios
        scenarios = self._extract_scenarios(content, relative_path, feature_tags)
    ...
    def _extract_scenarios(self, content: str, file_path: str, feature_tags: List[str]) -> List[Scenario]:
    ...
    def _associate_scenarios_with_requirements(self, scenarios: List[Scenario]):
    ...
    def _find_step_definition_files(self, feature_file: str, scenario_name: str) -> List[str]:
        """
    ...
    def generate_matrix(self):
        ...
        with open(self.output_file, 'w', encoding='utf-8') as f:
            ...
            f.write("## Matrix\n\n")
            f.write("| Requirement ID | Requirement Description | Feature File | Scenario(s) | Step Definition File(s) | Test Status | Coverage |\n")
            f.write("|----------------|-------------------------|--------------|-------------|-------------------------|------------|----------|\n")
            
            # Sort requirements by ID
            sorted_reqs = sorted(self.requirements.values(), key=lambda r: r.req_id)
            ...
            for req in sorted_reqs:
                ...
                for i, scenario in enumerate(req.scenarios):
                    ...
                    step_files = self._find_step_definition_files(scenario.feature_file, scenario.name)
                    step_files_set.update(step_files)
                
                # Format feature files (unique)
                feature_files = sorted(set(s.feature_file for s in req.scenarios))
                ...
                # Write the row
                f.write(f"| REQ-{req.req_id} | {req.description} | {feature_files_formatted} | {scenarios_formatted} | {step_files_formatted} | {status} | {req.coverage} |\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate a BDD traceability matrix')
    ...
    generator = TraceabilityMatrixGenerator(
        ...
        steps_dir=args.steps_dir or args.features_dir.replace('features', 'steps'),
