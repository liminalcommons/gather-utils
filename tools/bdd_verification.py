...
def run_pytest_bdd_tests():
    ...
    try:
        ...
        if result.stderr:
            ...
            if len(result.stderr) < 1000:  # Only print if it's reasonably short
                print("\nWarnings/Errors:")
                print(result.stderr)
        
        success = result.returncode == 0
        logger.info(f"Pytest BDD tests {'passed' if success else 'failed'} with return code {result.returncode}")
...
def run_behave_tests():
    ...
    try:
        ...
        success = result.returncode == 0
        logger.info(f"Behave tests {'passed' if success else 'failed'} with return code {result.returncode}")
...
def main():
    ...
    if overall_success:
        logger.info("Phase 5: Verification completed successfully - all tests passed")
    ...
    print(f"{'2' if not overall_success else '1'}. Proceed to Phase 6: Final Cleanup")
