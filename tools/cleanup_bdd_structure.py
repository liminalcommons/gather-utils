...
def cleanup_pytest_bdd_config():
    ...
    backup_file = (
        pytest_ini.parent
        / f"pytest.ini.bak.{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    for line in content:
        if not line.strip().startswith(
            "bdd_features_dir"
        ) and not line.strip().startswith("bdd_steps_dir"):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Clean up obsolete BDD test structure"
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    print(
        "This script will clean up the obsolete BDD test structure after migration."
    )
    ...
    print("\nCleanup Summary:")
    print(
        f"Features Directory: {'CLEANED' if features_cleaned else 'NO CHANGES'}"
    )
    print(
        f"pytest-bdd Configuration: {'CLEANED' if pytest_cleaned else 'NO CHANGES'}"
    )
