...
Dependencies: filecmp, logging, behave
Lifecycle:
...
def update_environment_py():
    ...
    try:
        ...
        if not missing_hooks:
            logger.info(
                f"No changes needed for environment.py - all required hooks present"
            )
            return False
...
"""
        ...
        logger.info(
            f"Updated environment.py with missing hooks: {', '.join(missing_hooks)}"
        )
        return True
