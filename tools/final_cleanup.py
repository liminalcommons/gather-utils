...
def verify_step_definitions():
    ...
    if not step_verification_script.exists():
        logger.warning(f"Step verification script not found: {step_verification_script}")
        print(f"Step verification script not found: {step_verification_script}")
        return False
        ...
        try:
            ...
            if is_empty:
                ...
                if choice.lower() == 'y':
                    ...
                    logger.info(f"Removed directory with contents: {directory}")
                    removed_count += 1
                else:
...
"""
        ...
        bdd_section_match = re.search(r'## BDD Testing\s.*?(?=\n##|\Z)', content, re.DOTALL)
        
        if bdd_section_match:
            # Replace existing section
            content = content[:bdd_section_match.start()] + bdd_section + content[bdd_section_match.end():]
        else:
...
def create_summary_file():
    ...
    summary_path = PROJECT_ROOT / "docs" / "project" / "BDD_CONSOLIDATION_SUMMARY.md"
    ...
    try:
        with open(summary_path, 'w', encoding='utf-8') as f:
            f.write("""# BDD Test Structure Consolidation Summary
...
def main():
    ...
    if create_summary_file():
        print("Created consolidation summary file at docs/project/BDD_CONSOLIDATION_SUMMARY.md")
    else:
    ...
    print("For more information, see the documentation at tests/bdd/README.md and")
    print("the consolidation summary at docs/project/BDD_CONSOLIDATION_SUMMARY.md.")
