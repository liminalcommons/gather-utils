...
def create_release_plan(release):
    ...
    cmd = ["python", "tools/repo_reconcile.py", "--milestone", milestone, "--create-bdd-scenarios"]
    ...
    with open(release_plan_file, "w") as f:
        ...
        if milestone == "Documentation":
            f.write("- [ ] Repository structure documented in a standardized format\n")
            f.write("- [ ] Redundancies identified and categorized by impact\n")
            f.write("- [ ] Development system map created for agent navigation\n")
            f.write("- [ ] BDD scenarios created for each improvement area\n")
            f.write("- [ ] Coverage report shows documentation requirements are met\n")
        elif milestone == "Structure":
            f.write("- [ ] Testing directories consolidated according to plan\n")
            ...
            f.write("- [ ] Repository health check enhanced with new metrics\n")
            ...
            f.write("- [ ] Agent can successfully navigate and modify the codebase\n")
            f.write("- [ ] BDD scenarios for agent requirements pass\n")
        
        f.write("\n## General Definition of Done\n\n")
        f.write("In addition to the specific acceptance criteria above, this release must meet all applicable criteria in the general Definition of Done document (DEFINITION_OF_DONE.md).\n")
...
def execute_release_tasks(release):
    ...
    if release != "1":
        ...
        prev_release_file = Path(f"reports/release_{prev_release}_complete.txt")
    ...
    if milestone == "Documentation":
        ...
        subprocess.run(["python", "tools/bdd_scenario_generator.py", "--milestone", milestone])
        ...
        subprocess.run(["python", "tools/repo_reconcile.py", "--execute", "--milestone", milestone])
        ...
        subprocess.run(["python", "tools/repo_health_check.py", "--validate-for-agent"])
...
def verify_release_acceptance_criteria(release):
    ...
    print("Verifying that the release meets all applicable criteria in the general Definition of Done...")
    ...
    release_status_file.write_text(f"Release {release} completed and acceptance criteria verified on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    print(f"Release {release} acceptance criteria verified and marked as complete")
    return True
...
def generate_release_report(release):
    ...
    with open(report_file, "w") as f:
        ...
        f.write("Based on this release, the following improvements to the general Definition of Done could be considered:\n\n")
        f.write("- [Add potential DoD improvements identified during this release]\n")
...
def main():
    ...
    parser.add_argument("--release", help="The release to manage (1, 2, 3, 4, or 'all')")
    parser.add_argument("--action", help="The action to perform (plan, execute, verify, report)")
    
    args = parser.parse_args()
    
    # Default to all releases if not specified
    releases = ["1", "2", "3", "4"] if args.release == "all" or not args.release else [args.release]
    
    # Default to all actions if not specified
    actions = ["plan", "execute", "verify", "report"] if not args.action else [args.action]
