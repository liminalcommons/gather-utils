...
def get_next_batch(batch_size=5):
    ...
    result = subprocess.run(
        [sys.executable, "tools/metadata_update/update_tracker.py", "--next-batch", "--batch-size", str(batch_size)],
        capture_output=True,
        text=True
    )
...
def analyze_tool(tool_path):
    ...
    """
    metadata = {
        "name": os.path.basename(tool_path).replace(".py", "").replace("_", " ").title(),
        "dependencies": "",
        "purpose": "",
        "created_reason": "",
        "active_status": "",
    ...
    if docstring_matches:
        for docstring in docstring_matches:
            ...
            purpose_match = re.search(r'(?:Purpose|This (?:script|tool|utility))(?:[\s:]*)([^\n\.]*(?:\.[^\n\.]*){0,2})', docstring, re.IGNORECASE)
            ...
            created_match = re.search(r'(?:Created to|Developed to|(?:^|\n)- Created to)(?:[\s:]*)([^\n\.]*(?:\.[^\n\.]*){0,2})', docstring, re.IGNORECASE)
    ...
    main_docstring_match = re.search(r'def main\(\):([\s\S]*?)"""([\s\S]*?)"""', content)
    ...
    for line in content.splitlines():
        if line.startswith("import ") or line.startswith("from "):
            ...
            if package not in imports and package not in ["os", "sys", "re", "json", "datetime", "pathlib", "argparse", "typing", "collections", "math", "time", "logging", "subprocess", "contextlib", "shutil", "tempfile", "functools", "itertools"]:
    ...
    if imports:
        ...
        req_match = re.search(r'(?:requirements|install_requires)[^\]]*\[(.*?)\]', content, re.DOTALL)
        if req_match:
            cleaned_reqs = re.sub(r'[\'",\s]', ' ', req_match.group(1))
            metadata["dependencies"] = ", ".join([r.strip() for r in cleaned_reqs.split() if r.strip()])
        else:
    ...
    if not metadata["purpose"]:
        ...
        for match in class_matches:
            ...
            if class_name.endswith("Analyzer") or class_name.endswith("Generator") or class_name.endswith("Manager"):
                sentences = re.split(r'(?<=[.!?])\s+', class_doc.strip())
                if sentences:
                    metadata["purpose"] = f"Provides a {class_name} to {sentences[0].lower().strip()}"
                    break
    ...
    if not metadata["purpose"]:
        ...
        if "bdd" in name_parts:
            if "coverage" in name_parts:
                metadata["purpose"] = "Generate coverage reports for BDD features and scenarios"
            elif "validator" in name_parts:
                metadata["purpose"] = "Validate BDD feature files for compliance with standards"
            elif "feature" in name_parts and "consolidator" in name_parts:
                metadata["purpose"] = "Consolidate similar or duplicate BDD features"
            elif "analyzer" in name_parts:
                metadata["purpose"] = "Analyze BDD features and scenarios for patterns and issues"
                ...
                metadata["purpose"] = "Generate documentation from BDD feature files"
            elif "test" in name_parts or "runner" in name_parts:
                metadata["purpose"] = "Run BDD tests and generate reports"
            else:
                metadata["purpose"] = "Support BDD testing and development workflows"
        elif "repo" in name_parts:
            if "health" in name_parts:
                metadata["purpose"] = "Check repository health and compliance with standards"
            elif "reconcile" in name_parts:
                metadata["purpose"] = "Reconcile repository structure with project standards"
            else:
                metadata["purpose"] = "Manage repository structure and organization"
        elif "tool" in name_parts and "inventory" in name_parts:
            metadata["purpose"] = "Manage and validate tool inventory and metadata"
        elif "metadata" in name_parts:
            if "injector" in name_parts:
                metadata["purpose"] = "Inject standardized metadata into tool files"
            elif "update" in name_parts and "tracker" in name_parts:
                metadata["purpose"] = "Track and report on metadata update progress"
            elif "processor" in name_parts:
                metadata["purpose"] = "Process multiple tools for metadata updates"
            else:
                metadata["purpose"] = "Manage tool metadata and standards compliance"
            ...
            metadata["purpose"] = "Set up development environment and dependencies"
        elif "fix" in name_parts and "style" in name_parts:
            metadata["purpose"] = "Fix code style issues according to project standards"
        else:
            metadata["purpose"] = f"Support {' '.join(name_parts)} functionality"
    
    # Determine created reason if not found in docstrings
    if not metadata["created_reason"]:
        if "bdd" in tool_path:
            metadata["created_reason"] = "To improve BDD testing capabilities and workflows"
        elif "metadata" in tool_path:
            metadata["created_reason"] = "To support metadata standardization across tools"
        elif "repo" in tool_path:
            metadata["created_reason"] = "To maintain repository health and standards"
        elif "docs" in tool_path or "documentation" in tool_path:
            metadata["created_reason"] = "To improve project documentation generation and management"
        elif "archive" in tool_path:
            metadata["created_reason"] = "To support historical functionality that may be needed for reference"
        else:
            metadata["created_reason"] = "To automate common development tasks and improve workflow efficiency"
    
    # Determine active status
    if "archive" in tool_path:
        metadata["active_status"] = "Archived for reference but no longer actively used"
    elif any(term in tool_path for term in ["metadata_update", "batch_processor", "update_tracker"]):
        metadata["active_status"] = "Used for metadata compliance and tracking"
    elif "bdd" in tool_path:
        metadata["active_status"] = "Used in BDD testing workflows and automation"
    elif "repo" in tool_path or "tool_inventory" in tool_path:
        metadata["active_status"] = "Used for repository maintenance and health checks"
    else:
...
def update_tool_metadata(tool_path, batch_id="auto"):
    ...
    result = subprocess.run([
        ...
        "--created-reason", metadata["created_reason"],
        "--active-status", metadata["active_status"],
        "--obsolescence", metadata["obsolescence"][0], metadata["obsolescence"][1]
