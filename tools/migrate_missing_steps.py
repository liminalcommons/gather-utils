...
def find_missing_steps(legacy_file, consolidated_dirs):
    ...
    logger.info(f"Found {len(legacy_steps)} steps in legacy file {legacy_file}")
            ...
            if isinstance(node, ast.FunctionDef):
                ...
                for decorator in node.decorator_list:
                    if (isinstance(decorator, ast.Call) and 
                        ...
                        decorator.func.id == step_type and 
                        len(decorator.args) > 0 and 
                        isinstance(decorator.args[0], ast.Constant) and 
                        decorator.args[0].value == step_pattern):
                        
                        # Extract the function code
                        function_lines = content.splitlines()[node.lineno-1:node.end_lineno]
                        return "\n".join(function_lines)
        
        logger.warning(f"Could not find implementation for @{step_type}('{step_pattern}') in {file_path}")
        return None
...
def migrate_steps(legacy_file, target_file, missing_steps):
    ...
    for step_type, step_pattern in missing_steps:
        implementation = extract_step_implementation(legacy_file, step_type, step_pattern)
        if implementation:
...
""")
    ...
    logger.info(f"Migrated {len(implementations)} step implementations to {target_file}")
...
def main():
    ...
    legacy_file = project_root / "features" / "steps" / "command_line_interface_steps.py.bak"
    ...
    # Find missing steps
    missing_steps, all_steps = find_missing_steps(legacy_file, [consolidated_dir])
    ...
    if missing_steps:
        ...
        if choice.lower() == 'y':
            ...
            print(f"Please review the migrated steps and make any necessary modifications.")
        else:
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        if missing_steps:
            if choice.lower() == 'y':
                f.write(f"✅ {count} steps have been migrated to `{target_file}`\n")
                if count < len(missing_steps):
                    f.write(f"⚠️ {len(missing_steps) - count} steps could not be migrated automatically\n")
            else:
