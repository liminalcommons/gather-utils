...
class StepDefinition:
    ...
    def _extract_step_functions(self):
        ...
        try:
                    ...
                    for decorator in node.decorator_list:
                        if isinstance(decorator, ast.Call) and hasattr(decorator.func, 'id'):
                            if decorator.func.id in ['given', 'when', 'then']:
                                ...
                                if decorator.args:
                                    step_pattern = decorator.args[0].s if hasattr(decorator.args[0], 's') else str(decorator.args[0])
            ...
            logger.error(f"Error extracting step functions from {self.path}: {e}")
...
def collect_step_definition_files():
    ...
    for directory in STEP_DIRECTORIES:
        ...
        for file_path in directory.glob("**/*.py"):
            ...
            if step_file.step_functions:  # Only include files with step functions
                step_files.append(step_file)
                logger.info(f"Found step definition file: {file_path} with {len(step_file.step_functions)} step functions")
...
def group_similar_step_files(step_files):
    ...
    for name, files in groups.items():
        ...
        for file in files:
            ...
            for group in content_groups:
                if file.similarity(group[0]) > 0.7 or file.has_overlapping_steps(group[0]):  # 70% similarity threshold or overlapping steps
                    group.append(file)
                    found_group = True
                    break
    ...
    while i < len(final_groups):
        ...
        while j < len(final_groups):
            if any(file1.has_overlapping_steps(file2) for file1 in final_groups[i] for file2 in final_groups[j]):
...
def migrate_step_definition_files(step_groups):
    ...
    for group in step_groups:
        ...
        if "tests/bdd/steps" in str(merged_file.path) and len(group) == 1:
            logger.info(f"File already in target directory: {merged_file.path}")
        ...
        try:
            ...
            logger.info(f"{'Merged' if len(group) > 1 else 'Migrated'} step definition file: {merged_file.path} -> {target_path}")
            
            # Update the path of the migrated file
            merged_file.path = target_path
            migrated_files.append(merged_file)
        except Exception as e:
            logger.error(f"Error writing step definition file {target_path}: {e}")
    
    logger.info(f"Migrated {len(migrated_files)} step definition files to {TARGET_DIRECTORY}")
...
def clean_up_obsolete_files(migrated_files, perform_cleanup=False):
    ...
    files_to_remove = [
        path for path in all_step_paths 
    ...
    if perform_cleanup:
        ...
        for file_path in files_to_remove:
            try:
                file_path.unlink()
                logger.info(f"Removed obsolete step definition file: {file_path}")
                removed_count += 1
            except Exception as e:
...
def report_migration_results(step_groups, migrated_files):
    ...
    print(f"Total step definition files found: {sum(len(group) for group in step_groups)}")
    print(f"Step definition file groups: {len(step_groups)}")
    print(f"Step definition files migrated to {TARGET_DIRECTORY}: {len(migrated_files)}")
...
def main(perform_cleanup=False):
    ...
    logger.info(f"Starting Phase 3: Step Definition Consolidation (cleanup={perform_cleanup})")
    ...
    logger.info(f"Grouped step definition files into {len(step_groups)} groups")
    ...
    print(f"{'3' if not perform_cleanup else '1'}. Proceed to Phase 4: Configuration Cleanup")

if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description="BDD Step Definition Consolidation")
