    ...
    for line in lines:
        ...
        if ":" in line and not line.startswith("-") and not line.startswith("#"):
            ...
            if len(parts) == 2:
                ...
                if key == "Lifecycle":
                    in_lifecycle = True
                    continue
                elif key in ["Created", "Last Updated", "Last Validated"] and len(value) >= 10:
                    # For date fields, extract YYYY-MM-DD
                    if value[0:4].isdigit() and value[5:7].isdigit() and value[8:10].isdigit():
        ...
        # Handle lifecycle section
            ...
            if line.startswith("- Created:"):
                ...
                lifecycle_data["Obsolescence Conditions"].append(line[2:].strip())
...
def validate_tool_metadata(file_path: str) -> Tuple[bool, Dict[str, Any], List[str]]:
    """
...
def generate_tool_inventory() -> pd.DataFrame:
    ...
    for file_path in tools:
        ...
        if "Lifecycle" in metadata:
            tool_data["Created Reason"] = metadata["Lifecycle"].get("Created", "Unknown")
            tool_data["Active Status"] = metadata["Lifecycle"].get("Active", "Unknown")
            tool_data["Obsolescence Conditions"] = "; ".join(metadata["Lifecycle"].get("Obsolescence Conditions", ["Unknown"]))
    ...
    df = df.sort_values(by=["Status", "Name"], key=lambda x: x.map({"Active": 0, "Deprecated": 1, "Archived": 2, "Unknown": 3} if x.name == "Status" else x))
    ...
    if format == "html":
        ...
        for status in ["Active", "Deprecated", "Archived", "Unknown"]:
            ...
            if len(status_df) > 0:
                ...
                status_html = status_df.to_html(index=False, escape=False, classes="table")
                
                # Replace values with styled versions
                status_html = status_html.replace('>True<', ' class="metadata-valid">Valid<')
                status_html = status_html.replace('>False<', ' class="metadata-invalid">Invalid<')
                status_html = status_html.replace(f'>{status}<', f' class="status-{status.lower()}">{status}<')
        ...
        with open(md_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"- Deprecated Tools: {len(df[df['Status'] == 'Deprecated'])}\n")
            f.write(f"- Archived Tools: {len(df[df['Status'] == 'Archived'])}\n")
            f.write(f"- Tools with Invalid Metadata: {len(df[df['Metadata Valid'] == False])}\n\n")
            
            for status in ["Active", "Deprecated", "Archived", "Unknown"]:
                status_df = df[df["Status"] == status]
                if len(status_df) > 0:
                    f.write(f"## {status} Tools\n\n")
                    f.write(tabulate(status_df, headers="keys", tablefmt="pipe"))
                    f.write("\n\n")
...
def search_tools(query: str, inventory_df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    """
    ...
    mask = False
    for col in ["Name", "Purpose", "Created Reason", "Active Status", "Obsolescence Conditions"]:
        if col in inventory_df.columns:
            mask = mask | inventory_df[col].str.lower().str.contains(query, na=False)
...
def archive_tool(tool_path: str, reason: str) -> bool:
    ...
    try:
        ...
        if os.path.exists(dest_path):
            print(f"{COLOR_YELLOW}Warning: {filename} already exists in archive.{COLOR_RESET}")
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            dest_path = os.path.join(abs_archive_path, f"{os.path.splitext(filename)[0]}_{timestamp}{os.path.splitext(filename)[1]}")
        ...
        if metadata_str:
            ...
            # Add archival note
            archive_note = f'Archived: {datetime.now().strftime("%Y-%m-%d")} - {reason}'
        ...
        print(f"{COLOR_GREEN}Successfully archived {filename} to {dest_path}{COLOR_RESET}")
        
        # Remove the original file
        os.remove(abs_tool_path)
        print(f"{COLOR_GREEN}Removed original file at {abs_tool_path}{COLOR_RESET}")
...
def mark_deprecated(tool_path: str, reason: str, replacement: Optional[str] = None) -> bool:
    """
    ...
    try:
        ...
        if not metadata_str:
            print(f"{COLOR_RED}Error: No metadata found in {tool_path}{COLOR_RESET}")
            return False
        ...
        deprecation_note = f'Deprecated: {datetime.now().strftime("%Y-%m-%d")} - {reason}'
        if replacement:
        ...
        print(f"{COLOR_GREEN}Successfully marked {os.path.basename(tool_path)} as deprecated{COLOR_RESET}")
        return True
        
    except Exception as e:
        print(f"{COLOR_RED}Error marking {tool_path} as deprecated: {e}{COLOR_RESET}")
        return False
...
def update_tool_readme(inventory_df: pd.DataFrame) -> bool:
    ...
    try:
        ...
        for _, row in inventory_df[inventory_df["Status"] == "Active"].iterrows():
            tool_name = row["Name"]
            purpose = row["Purpose"]
            path = row["Path"]
            ...
            if "BDD" in tool_name or "bdd" in path:
                ...
                category = "TDD Tools"
            elif any(kw in purpose.lower() for kw in ["style", "lint", "format"]):
        ...
        for _, row in inventory_df[inventory_df["Status"] == "Archived"].iterrows():
        ...
        if "## Available Tools" in readme_content and "## Usage" in readme_content:
            start_idx = readme_content.find("## Available Tools")
            end_idx = readme_content.find("## Usage", start_idx)
            
            updated_readme = readme_content[:start_idx] + available_tools_section + readme_content[end_idx:]
        else:
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--validate",
        action="store_true",
        help="Validate tool metadata"
    )
    
    parser.add_argument(
        "--search",
        type=str,
        help="Search for tools matching the given query"
    )
    ...
    parser.add_argument(
        "--reason",
        type=str,
        help="Reason for archiving or deprecating a tool"
    )
...
def main() -> int:
    ...
    if args.inventory:
        ...
        print(f"{COLOR_GREEN}Inventory report saved to {report_file}{COLOR_RESET}")
    
    if args.validate:
        valid_count = len(inventory_df[inventory_df["Metadata Valid"] == True])
        invalid_count = len(inventory_df[inventory_df["Metadata Valid"] == False])
        ...
        if invalid_count > 0:
            ...
            for _, row in inventory_df[inventory_df["Metadata Valid"] == False].iterrows():
    ...
    if args.search:
        ...
        if len(results_df) > 0:
            print(f"{COLOR_GREEN}Found {len(results_df)} tools matching '{args.search}':{COLOR_RESET}")
            print(tabulate(results_df[["Name", "Status", "Purpose", "Path"]], headers="keys", tablefmt="grid"))
        else:
            print(f"{COLOR_YELLOW}No tools found matching '{args.search}'{COLOR_RESET}")
    
    if args.archive:
        if not args.reason:
            print(f"{COLOR_RED}Error: --reason is required when archiving a tool{COLOR_RESET}")
            return 1
    ...
    if args.deprecate:
        if not args.reason:
            print(f"{COLOR_RED}Error: --reason is required when deprecating a tool{COLOR_RESET}")
            return 1
        
        success = mark_deprecated(args.deprecate, args.reason, args.replacement)
        if not success:
            return 1
    ...
    if not any([args.inventory, args.validate, args.search, args.archive, args.deprecate, args.update_readme]):
        print(f"{COLOR_BLUE}Tool Inventory Manager{COLOR_RESET}")
        print(f"{COLOR_YELLOW}No action specified. Use --help to see available options.{COLOR_RESET}")
        print("\nQuick summary:")
        print(f"Total tools: {len(inventory_df)}")
        print(f"Active: {len(inventory_df[inventory_df['Status'] == 'Active'])}")
        print(f"Deprecated: {len(inventory_df[inventory_df['Status'] == 'Deprecated'])}")
        print(f"Archived: {len(inventory_df[inventory_df['Status'] == 'Archived'])}")
        print(f"Unknown: {len(inventory_df[inventory_df['Status'] == 'Unknown'])}")
        print(f"Invalid metadata: {len(inventory_df[inventory_df['Metadata Valid'] == False])}")
